...
internal static class ShipServiceRecords
{
    ...
    internal static void RecordBattle(List<BattleResult> results, string dateLabel)
    {
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP ship records: logged {newEntries} ship-battle entr(ies) ({recs.Count} ship(s) tracked) for {dateLabel}.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP ship records: record failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static Dictionary<string, Record> Load()
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP ship records: load failed — {ex.GetType().Name}: {ex.Message}");
        }
        return recs;
    }
