...
// At campaign battle end the VP-winner takes ALL surrendered ships (capture the loser's,
...
internal static class SurrenderedShipCapture
{
    ...
    internal static void OnCompleteBattle(CampaignBattle battle)
    {
        ...
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP surrendered capture: victor={PlayerName(victor)} queued {queued} surrendered ship(s) for post-battle reinstatement.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP surrendered capture (queue) failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void ReinstatePending()
    {
        ...
        try
        {
            ...
            foreach (Pending p in batch)
            {
                ...
                if (victor == null)
                {
                    Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP capture reinstate '{p.Name}': victor not resolvable, skipped.");
                    continue;
                }
                ...
                {
                    Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP capture reinstate '{p.Name}' threw: {ex.GetType().Name}: {ex.Message}");
                    continue;
                }
                ...
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP capture reinstate '{p.Name}' -> {PlayerDataName(p.Victor)} resolved={resolved}{extra}.");
            }

            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP capture reinstate: {ok}/{batch.Count} surrendered ship(s) transferred to the victor.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP capture reinstate failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void ProtectFromLossPass(CampaignBattle battle)
    {
        ...
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP loss-pass protect: removed {removed} kept ship(s) from battle loss lists (protected={_protect.Count}).");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP loss-pass protect failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void LogLossPass(string tag)
    {
        if (_protect.Count == 0)
            return;
        Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP loss-pass {tag}: mainVessels={MainVesselCount()} protected={_protect.Count}.");
    }
