...
// the selected method, a UADVP_TOPO line is logged whenever player division membership changes, so a
...
internal static class BattleTurn
{
    ...
    internal static void TryHotkey(Ui? ui)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP reverse-course failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
        ...
        try
        {
            ...
            if (turned > 0)
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP reverse-course [{ModSettings.BattleTurnMethodText(method)}]: {turned} division(s) to {side}.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP reverse-course failed: {ex.GetType().Name}: {ex.Message}");
    ...
    private static void Log(string msg, string prefix = "UADVP_TURNLOG ") => Melon<UADVanillaPlusMod>.Logger.Msg(prefix + msg);
