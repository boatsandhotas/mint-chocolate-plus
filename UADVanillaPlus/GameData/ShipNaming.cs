    ...
    internal static string RenameClassToTheme(string baseKey, string themeName, string nation)
    {
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP rename class '{baseKey}' -> theme '{themeName}': {familyDesigns.Count} design(s), {classShips.Count} ship(s), {renamed} renamed.");
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP class rename failed: {ex.GetType().Name}: {ex.Message}");
            return string.Empty;
        }
    }
