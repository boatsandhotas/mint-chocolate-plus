...
internal static class PatrolPlanner
{
    ...
    internal static PatrolPlan Compute()
    {
            ...
            foreach (Ship s in human.GetFleetAll())
            {
                ...
                if (dumpOnce && dbgCount < 200)
                {
                    ...
                    Melon<UADVanillaPlusMod>.Logger.Msg(
                        $"UADVP_PATROLDBG class=\"{clsLabel}\" type={Safe(() => s.shipType?.name, "?")} owner={Safe(() => s.player?.data?.name, "?")} isMain={mine} forSale={forSale} saleTo={Safe(() => s.ForSaleTo?.name, "-")} building={Safe(() => s.isBuilding, false)} sailing={sailing} grouped={grouped} port={Safe(() => s.PortLocation?.Id, "-")}");
                }
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP patrol pool enumeration failed: {ex.GetType().Name}: {ex.Message}");
        }
