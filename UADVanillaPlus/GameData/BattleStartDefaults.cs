...
internal static class BattleStartDefaults
{
    ...
    private const string ConfigKey = "uadvp_battle_start_pertype";
    ...
    internal static void Apply()
    {
        ...
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP battle-start defaults applied (per-type): divisions={divCount} ships set={shipCount}.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP battle-start defaults failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void ReapplyNewDivisions()
    {
        ...
        try
        {
            ...
            if (newDivs > 0)
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP battle-start defaults: re-applied to {newDivs} new division(s) (e.g. after a split).");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP battle-start reapply failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
