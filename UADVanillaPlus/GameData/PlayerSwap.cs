...
// isMain/isAi on both nations, carry the campaign-id GUID (the human's aiName, which VP keys its
...
internal static class PlayerSwap
{
    ...
    internal static void SwitchTo(Player target)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP nation switch: cannot switch — {reason}.");
            return;
        }
        if (target == null)
            return;
        ...
        if (oldHuman == null)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning("UADVP nation switch: no current human player found.");
            return;
        }
        try { if (target.Pointer == oldHuman.Pointer) { Melon<UADVanillaPlusMod>.Logger.Warning("UADVP nation switch: already controlling that nation."); return; } } catch { }
        ...
        try
        {
            // Carry the campaign-id GUID to the new human FIRST so VP per-campaign state stays keyed
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP nation switch: flag swap failed — {ex.GetType().Name}: {ex.Message}. Aborting before save.");
            return;
        }

        Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP nation switch: {oldName} -> {newName}. Saving and returning to main menu — click Continue to resume as {newName}.");

        try { GameManager.Save(true, true); }
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP nation switch: save failed — {ex.GetType().Name}: {ex.Message}."); return; }
        ...
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP nation switch: to-main-menu failed — {ex.GetType().Name}: {ex.Message}. The switch is saved; load the campaign manually."); }
    }
