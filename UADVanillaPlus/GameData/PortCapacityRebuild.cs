...
internal static class PortCapacityRebuild
{
    ...
    internal static void Reconcile(CampaignController cc)
    {
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP shipyard-rebuild: reconciled {ModCampaignState.DebugIdentity()} schedules={_schedules.Count} enabled={ModSettings.MultiYearShipyardRebuildEnabled}.");
        }
        ...
        Dictionary<string, double> beforeWeighted, Dictionary<string, double> beforeShipyard)
    {
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg(
            $"UADVP shipyard-rebuild capture prov={prov} {loser}->{captor} amount={amount:0} dev={pi.Dev:0.0} home={pi.IsHome && string.Equals(pi.Claim, loser, StringComparison.Ordinal)} durYears={durationYears:0.0} loserYard->{SafeFloat(() => loserP.shipyard):0}.");
    }
    ...
    private static void AdvanceSchedules(int turn, Dictionary<string, Player> players)
    {
        ...
        foreach (var kv in _schedules)
        {
            ...
            if (target >= s.Total)
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP shipyard-rebuild complete prov={kv.Key} player={s.ToPlayer} total={s.Total:0}.");
        }
    }
    ...
    private static void Warn(string where, Exception ex)
        => Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP shipyard-rebuild {where} failed: {ex.GetType().Name}: {ex.Message}");
