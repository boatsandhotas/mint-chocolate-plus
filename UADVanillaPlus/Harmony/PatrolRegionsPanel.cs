...
//           (built -> MoveVessels, building -> field write), which logs every move (UADVP_VASSIGN).
//
...
internal static class PatrolRegionsPanel
{
    ...
    internal static void TryHotkey()
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP patrol panel: hotkey failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    {
        GameObject canvasGo = new("UADVP_PatrolCanvas");
        ...
        panel = new GameObject("UADVP_PatrolPanel");
    ...
    private static void SendSelected()
    {
        try
        {
            ...
            {
                Melon<UADVanillaPlusMod>.Logger.Msg("UADVP_VASSIGN patrol: no region selected.");
                return;
            }
            var vessels = SelectedVessels();
            if (vessels.Count == 0)
            {
                Melon<UADVanillaPlusMod>.Logger.Msg("UADVP_VASSIGN patrol: no ships selected.");
                return;
            }
            PortElement dest = ResolveDest(region);
            int done = VesselAssignment.AssignManyToPort(vessels, dest);
            Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_VASSIGN patrol send: {done}/{vessels.Count} ship(s) -> {region.Label} / {SafeStr(() => dest.Id)}.");
            selectedShips.Clear();
            Recompute();
            RefreshAll();
        }
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP patrol send failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void AutoDistribute()
    {
        try
        {
            ...
            if (vessels.Count == 0)
            {
                Melon<UADVanillaPlusMod>.Logger.Msg("UADVP_VASSIGN patrol auto: no ships selected.");
                return;
            }
            if (plan.Regions.Count == 0)
            {
                Melon<UADVanillaPlusMod>.Logger.Msg("UADVP_VASSIGN patrol auto: no regions.");
                return;
            }
            ...
            foreach (VesselEntity v in vessels)
            {
                ...
                work[bestIdx] += ton;
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_VASSIGN patrol auto: ship -> {region.Label} (need was {bestNeed:0}) ok={ok}");
            }

            Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_VASSIGN patrol auto-distribute: {done}/{vessels.Count} dispatched.");
            selectedShips.Clear();
            Recompute();
            RefreshAll();
        }
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP patrol auto-distribute failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
