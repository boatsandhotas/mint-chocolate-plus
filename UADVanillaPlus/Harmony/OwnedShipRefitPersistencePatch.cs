...
internal static class OwnedShipRefitPersistence
{
    ...
    internal static bool ShouldSkipMainPlayerCull(Player? player)
    {
        ...
        if (!loggedCullSkip)
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                "UADVP owned-ship refit: skipping obsolescence cull (DeleteOldDesigns) for the main player so owned designs are not auto-erased (status 7). Manual deletes are unaffected.");
        }
    ...
    internal static void UneraseIfNeeded(Ship? design, string context)
    {
        ...
        try
        {
            ...
            if (uneraseLogCount < 60)
            {
                uneraseLogCount++;
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP owned-ship refit: un-erased design '{SafeName(design)}' (status Erased->None) at {context} so the refit can persist.");
            }
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP owned-ship refit: un-erase failed at {context}: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void UneraseRefitClones(string context)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP owned-ship refit: refit-clone scan failed at {context}: {ex.GetType().Name}: {ex.Message}");
        }
    }
...
internal static class OwnedShipRefitCullSkipPatch
{
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<UADVanillaPlusMod>.Logger.Warning("UADVP owned-ship refit: DeleteOldDesigns not found; cull-skip disabled.");
        return available;
    }
