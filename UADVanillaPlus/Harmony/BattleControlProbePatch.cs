...
// battle-turn, pre-battle divisions, compass-heading). Driven from VP's already-working
...
internal static class BattleControlProbe
{
    ...
    internal static void DumpDivisions(string ctx)
    {
        try
        {
            ...
            if (divisions == null)
            {
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_BATTLEPROBE [{ctx}] no player divisions.");
                return;
            }
            ...
            foreach (Division d in divisions)
            {
                ...
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP_BATTLEPROBE [{ctx}] div{i} leader={Name(lead)} leaderSet={F(() => lead != null ? lead.engineCustomSpeed : -1f):0.0} ships=[{string.Join(", ", parts)}]");
                i++;
            }
            if (i == 0)
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_BATTLEPROBE [{ctx}] 0 player divisions in list.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_BATTLEPROBE [{ctx}] failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
