...
// CampaignController being present. Log prefix for Plan/Rebalance: "UADVP_MINEREBAL".
internal static class MinelayerGoalsPanel
{
    ...
    internal static void TryHotkey()
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP minelayer goals: hotkey failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    {
        GameObject canvasGo = new("UADVP_MinelayerGoalsCanvas");
        ...
        panel = new GameObject("UADVP_MinelayerGoalsPanel");
    ...
    {
        Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL === {(applied ? "REBALANCED" : "PLAN")} idle={plan.IdleTotal} assigned={plan.AssignedTotal} deployed={plan.DeployedTotal} placements={plan.Moves.Count} ===");
        int shown = 0;
        foreach (MinelayerPlanner.Move m in plan.Moves)
        {
            if (shown++ >= 80) break;
            Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL   {(applied ? "PLACED" : "PLAN")} \"{m.Name}\" [{m.Variant}] -> {m.ToPort}");
        }
        foreach (var kv in plan.Shortfall)
            Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL   shortfall(build) [{kv.Key}] = {kv.Value}");
    }
    ...
    private static void DoRebalance()
    {
        try
        {
            ...
            if (moves.Count == 0)
            {
                Melon<UADVanillaPlusMod>.Logger.Msg("UADVP_MINEREBAL nothing to place (no idle subs or no deficit).");
                return;
            }
            int applied = MinelayerPlanner.Apply(moves);
            Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL applied {applied}/{moves.Count} placement(s).");
            LogPlan(true);
            Recompute();
            RefreshLeft();
            RefreshPortList();
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP minelayer goals: rebalance failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void BuildAndAssign(string variant)
    {
        try
        {
            ...
            {
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL no SubmarineType for {variant} — build one manually first.");
                return;
            }
            ...
            if (slots.Count == 0)
            {
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL {variant}: no deficit to fill.");
                return;
            }
            ...
            if (plan.ShipyardCapacity > 0f && need > 0 && projected > plan.ShipyardCapacity)
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP_MINEREBAL WARNING: building {need} {variant} (+{addTon:0}t) -> shipyard ~{projected:0}t / {plan.ShipyardCapacity:0}t, OVER by {projected - plan.ShipyardCapacity:0}t (builds will be slower).");
            ...
            if (need > 0)
            {
                ...
                if (!can)
                {
                    Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL cannot build {variant}: {reason} — assigning idle only.");
                }
                else
                {
                    try
                    ...
                    {
                        Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_MINEREBAL build call failed: {bex.GetType().Name}: {bex.Message}");
                    }
                }
            }
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_MINEREBAL build+assign [{variant}]: deficit={slots.Count} idle={idleN} built={built} assigned={assigned}.");
            Recompute();
            RefreshLeft();
            RefreshPortList();
        }
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP minelayer goals: build+assign failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
