...
internal static class CampaignAiEconomyPrioritiesPatch
{
    ...
    private static void AiCrewTrainingBudgetPostfix(Player player)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP AI economy priorities failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogOnce(Player player, float cap, float bTr, float bTech, float bCrew, float maxCrew)
    {
        try
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP_ECONLOG {name}: transportCap={cap:0.000} budgets[tr={bTr:0.##} tech={bTech:0.##} crew={bCrew:0.##}] maxCrew={maxCrew:0.##} navalBudget={budget:0} stateBudget={state:0}");
        }
        catch { }
    }
