...
// that drives VP's per-turn campaign systems in a guarded, deterministic order
...
internal static class CampaignPerTurnDispatchPatch
{
    ...
    private static void OnNewTurnPostfix(CampaignController __instance)
    {
        ...
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP per-turn dispatch: {ex.GetType().Name}: {ex.Message}"); }
    ...
    private static void DisablePlayerPrefix(CampaignController __instance, Player player)
    {
        ...
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP vanquished dispatch: {ex.GetType().Name}: {ex.Message}"); }
    }
}
