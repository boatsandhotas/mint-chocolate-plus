...
// Diagnostics, watch UADVP_FOLLOWLOG through a torpedo engagement, and see whether ships coming off an
...
// Driven from VP's existing Ui.Update postfix (alongside BattleTurn). Gated behind Battle Runtime
    ...
    private static void Dump()
    {
        ...
        foreach (Division d in divisions)
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP_FOLLOWLOG div={DivId(d)} damp={(ModSettings.FollowSteerDampingEnabled ? "On" : "Off")} lead={Name(lead)}:hdg={D(leadHdg)}/av={(lead != null ? AV(lead) : "?")} foll=[{string.Join(", ", parts)}]");
        }
    }
