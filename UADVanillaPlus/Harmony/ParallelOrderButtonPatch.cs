...
// Sits just after the AI control button so the two VP buttons don't overlap. Refreshed from the same
// Ui.RefreshShipControls postfix as the AI button.
internal static class ParallelOrderButton
{
    private const string ButtonName = "UADVP_OrderParallel";
    private const string AiButtonName = "UADVP_OrderAiControl";
    ...
    internal static void Refresh(Ui ui)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP parallel button failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static Button? Ensure(BattleDivisionAiControlPatch.OrderButtonContext ctx, List<(string SourceName, GameObject Root)> roots)
    {
        ...
        {
            loggedAdded = true;
            Melon<UADVanillaPlusMod>.Logger.Msg("UADVP parallel order: added Parallel order button to battle division orders.");
        }
        return button;
    }
