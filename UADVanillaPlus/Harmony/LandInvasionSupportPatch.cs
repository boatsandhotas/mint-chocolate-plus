...
internal static class LandInvasionSupportHotkeys
{
    ...
    private static void Postfix()
    {
        ...
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_REINFORCE hotkey err {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static void CommitCurrent()
    {
        ...
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_REINFORCE commit err {ex.GetType().Name}: {ex.Message}"); }
    }
}
