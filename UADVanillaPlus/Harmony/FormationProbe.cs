        ...
        try
        {
            ...
            foreach (Division d in divisions)
            {
                ...
                try
                {
                    ...
                    {
                        LastFormation[key] = fv;
                        Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_FORMLOG div={DivId(d)} formation CHANGED -> {form}");
                    }
                }
                catch { }
                ...
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP_FORMLOG div={DivId(d)} form={form} lead={Name(lead)} [{string.Join(", ", parts)}]");
            }
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_FORMLOG failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
