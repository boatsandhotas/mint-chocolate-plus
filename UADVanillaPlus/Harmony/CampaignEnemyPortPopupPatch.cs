...
// because PortPopupUI.Show never fires. VP appends a second onClick listener
...
internal static class CampaignEnemyPortPopupPatch
{
        ...
        try
        {
            ...
            if (attached > 0 || forced > 0)
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP enemy-port-popup: attached={attached} forced-interactable={forced} " +
                    $"portUiCount={totalPortUis} elementCount={totalSeen}.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP enemy-port-popup: AttachAll failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void OnPortClicked(PortUI portUi, Button button)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP enemy-port-popup: click portUi.Id='{portUi.Id}' provinceId='{portUi.ProvinceId}' " +
            ...
            {
                Melon<UADVanillaPlusMod>.Logger.Warning(
                    $"UADVP enemy-port-popup: could not resolve PortElement for PortUI id='{portUi.Id}' provinceId='{portUi.ProvinceId}'.");
                return;
            }
            ...
            if (popup == null)
            {
                Melon<UADVanillaPlusMod>.Logger.Warning("UADVP enemy-port-popup: both PortWindow and PortPopup are null.");
                return;
            }
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP enemy-port-popup: calling Show on {popupName} for port {port.Id} " +
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(
                $"UADVP enemy-port-popup: after Show, popup.Root.activeSelf={(root != null ? root.activeSelf.ToString() : "<null>")}.");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP enemy-port-popup: click handler failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
