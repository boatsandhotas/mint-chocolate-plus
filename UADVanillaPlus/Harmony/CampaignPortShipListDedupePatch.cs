...
internal static class CampaignPortShipListDedupePatch
{
    ...
    private static void Postfix(PortPopupUI __instance)
    {
        try
        {
            ...
            if (hidden > 0)
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP port ship-list: hid {hidden} duplicate row(s).");
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP port ship-list dedupe failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
