...
// turn ticks. VP adds a Launch Invasion button to the port popup so the player
...
{
    private const string LaunchInvasionButtonName = "UADVP_LaunchInvasion";
    ...
    private static void Postfix(PortPopupUI __instance, PortElement port)
    {
            ...
            if (source == null)
            {
                ...
                if (!LoggedNoSourceForPopup.Contains(__instance.Pointer))
                {
                    ...
                    Melon<UADVanillaPlusMod>.Logger.Msg(
                        $"UADVP launch-invasion: no source button on PortPopupUI (instance={which}, " +
        ...
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP launch-invasion: PortPopupUI.Show postfix failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static Button? EnsureLaunchButton(PortPopupUI popup, Button source)
    {
        ...
        // so VP fully owns the label/tooltip surface.
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg(
            $"UADVP launch-invasion: added Launch Invasion button to PortPopupUI (cloned from {source.name}).");
        return button;
    }
    ...
    private static void AttachTooltip(GameObject target, string text)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP launch-invasion: tooltip attach failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LayoutButtonRow(PortPopupUI popup, Button launchButton)
    {
        try
        {
            ...
            {
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP launch-invasion: PRE parent.rect.width={(parentRect != null ? parentRect.rect.width : -1f):0} " +
                ...
                foreach (Button b in active)
                {
                    ...
                    if (rr == null) continue;
                    Melon<UADVanillaPlusMod>.Logger.Msg(
                        $"UADVP launch-invasion: PRE {b.name} rect.width={rr.rect.width:0} " +
            ...
            // the parent).
            System.Text.StringBuilder sb = new("UADVP launch-invasion: post-layout widths");
            float total = 0f;
            ...
            Melon<UADVanillaPlusMod>.Logger.Msg(sb.ToString());
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP launch-invasion: LayoutButtonRow failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
        ...
        string reason)
    {
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP launch-invasion button {state}");
    }
