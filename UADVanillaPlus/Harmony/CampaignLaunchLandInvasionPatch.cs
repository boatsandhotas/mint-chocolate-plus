...
internal static class CampaignLaunchLandInvasionPatch
{
    ...
    private const string CloseProvincePopupButtonName = "UADVP_CloseProvincePopup";
    ...
    private static void Postfix(CampaignProvincePopupUI __instance, string provinceId)
    {
        try
        {
            ...
            if (lastShownLoggedProvince != province.Id)
            {
                ...
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP land-invasion: Show postfix for province={province.Id} controller={controller} " +
            ...
            if (!clickContext)
            {
                // Hide any previously-added VP button so a tooltip from hover
        ...
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP land-invasion: CampaignProvincePopupUI.Show postfix failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static Button? EnsureLaunchButton(CampaignProvincePopupUI popup)
    {
        ...
        if (parent == null)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning("UADVP land-invasion: CampaignProvincePopupUI.LayoutRoot is null.");
            return null;
        }
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                "UADVP land-invasion: no button template available yet; will retry on next popup open.");
            return null;
        }
        ...
        // Strip inherited vanilla behaviour: VP owns the label, color, and tooltip.
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg(
            $"UADVP land-invasion: added Launch Land Invasion button to CampaignProvincePopupUI (cloned from {template.name}).");
        return button;
    }
    ...
    private static Button? EnsureCloseButton(CampaignProvincePopupUI popup)
    {
        ...
        button.onClick.AddListener(new System.Action(() =>
        {
            ...
            {
                Melon<UADVanillaPlusMod>.Logger.Warning(
                    $"UADVP land-invasion: Close click failed. {ex.GetType().Name}: {ex.Message}");
            }
        }));

        Melon<UADVanillaPlusMod>.Logger.Msg(
            "UADVP land-invasion: added Close button to province popup.");
        return button;
    }
    ...
    private static Button? LocateButtonTemplate()
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP land-invasion: button template lookup failed. {ex.GetType().Name}: {ex.Message}");
        }
        return null;
    }
    ...
    private static void ConfirmLaunch(CampaignProvincePopupUI popup)
    {
        ...
        MessageBoxUI.Show(
            ...
            {
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP land-invasion: confirm-Yes received for province={province.Id}; calling LaunchLandInvasion…");
                bool launched = false;
                try
                {
                ...
                {
                    Melon<UADVanillaPlusMod>.Logger.Warning(
                        $"UADVP land-invasion: confirm-Yes LaunchLandInvasion threw. {ex.GetType().Name}: {ex.Message}");
                }
                Melon<UADVanillaPlusMod>.Logger.Msg(
                    $"UADVP land-invasion: LaunchLandInvasion returned launched={launched}.");
                if (launched)
                {
                    try
                    {
                        ProvincePopupInvocationContext.Unpin();
                        popup.Hide();
                        Melon<UADVanillaPlusMod>.Logger.Msg("UADVP land-invasion: popup unpinned + hidden.");
                    }
                    catch (Exception ex)
                    {
                        Melon<UADVanillaPlusMod>.Logger.Warning(
                            $"UADVP land-invasion: Unpin/Hide threw. {ex.GetType().Name}: {ex.Message}");
                    }
                }
                else
                {
        ...
        string reason)
    {
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP land-invasion button {state}");
    }
