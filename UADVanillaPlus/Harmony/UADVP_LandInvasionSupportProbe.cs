...
//
// Trigger: Ctrl+Shift+L on the campaign map. Output goes to the MelonLoader log (grep "UADVP_LANDPROBE").
[HarmonyPatch(typeof(Cam), "Update")]
internal static class UADVP_LandInvasionSupportProbe
{
    [HarmonyPostfix]
    private static void Postfix()
    {
        ...
        catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_LANDPROBE failed {ex.GetType().Name}: {ex.Message}"); }
    }

    private static void Probe()
    {
        MelonLogger.Instance L = Melon<UADVanillaPlusMod>.Logger;
        L.Msg("=== UADVP_LANDPROBE (Step-0, read-only) ===");
