...
//
// Reads the per-campaign data layer (UADVanillaPlus.GameData.ShipServiceRecords) and presents it as a
...
// not depend on private helpers in other VP files. All engine calls are try/catch wrapped for IL2CPP
...
internal static class ShipRecordsViewer
{
    ...
    internal static void TryHotkey()
    {
        ...
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP ship records viewer: hotkey failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void Toggle()
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP ship records viewer: toggle failed — {ex.GetType().Name}: {ex.Message}");
            Close();
        }
    }
    ...
    private static void Open()
    {
        if (!ModSettings.ShipServiceRecordsEnabled)
        {
            Melon<UADVanillaPlusMod>.Logger.Msg("UADVP ship records viewer: feature is Off; not opening.");
            return;
        }
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg(
            $"UADVP ship records viewer: opened ({records.Count} ship(s) tracked).");
    }
    ...
    private static void Close()
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP ship records viewer: close failed — {ex.GetType().Name}: {ex.Message}");
        }
        finally
        {
            canvas = null;
    ...
    private static void LoadRecords()
    {
        ...
        catch (Exception ex)
        {
            ...
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP ship records viewer: load failed — {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static void SetSort(SortMode mode)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP ship records viewer: sort failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    {
        GameObject canvasGo = new("UADVP_ShipRecordsCanvas");
        ...
        panel = new GameObject("UADVP_ShipRecordsPanel");
        ...
        GameObject body = new("UADVP_ShipRecordsBody");
        ...
        GameObject left = new("UADVP_ShipListColumn");
        ...
        GameObject listScroll = BuildScroll(left.transform, "UADVP_ShipList", out Transform listContent);
        ...
        GameObject right = new("UADVP_ShipRecordsDetail");
    ...
    {
        GameObject header = new("UADVP_ShipRecordsHeader");
    ...
    private static void BuildSortRow(Transform parent)
    {
        ...
        GameObject row = new("UADVP_SortRow");
    ...
    {
        GameObject go = new($"UADVP_Sort_{mode}");
    ...
    private static void PopulateList(Transform content)
    {
        ...
        foreach (ShipServiceRecords.Record rec in ordered)
        {
            ...
            GameObject row = new("UADVP_ShipRow");
    ...
    private static void OnSelect(string id)
    {
        ...
        {
            Melon<UADVanillaPlusMod>.Logger.Warning(
                $"UADVP ship records viewer: select failed — {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void BuildDetail(Transform parent)
    {
        ...
        GameObject battleScroll = BuildScroll(parent, "UADVP_BattleList", out Transform battleContent);
        ...
        for (int i = rec.Battles.Count - 1; i >= 0; i--)
        {
            ...
            GameObject rowGo = new("UADVP_BattleRow");
    ...
    // ----- self-contained UI helpers (no dependencies on other VP files) -----
    ...
    {
        GameObject go = new($"UADVP_Btn_{label}");
        go.transform.SetParent(parent, false);
        Image img = go.AddComponent<Image>();
        img.color = CloseBtn;
        img.raycastTarget = true;
...
// Trigger: campaign hotkey. A second postfix on Ui.Update alongside other VP patches is fine —
