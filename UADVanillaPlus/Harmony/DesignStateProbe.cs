...
internal static class DesignStateProbe
{
    ...
    internal static void Tick()
    {
        try
        {
            ...
            catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_DESIGNPROBE designs-enum failed: {ex.GetType().Name}: {ex.Message}"); return; }

            if (!loggedScan)
            {
                loggedScan = true;
                Melon<UADVanillaPlusMod>.Logger.Msg($"UADVP_DESIGNPROBE scanning {designs.Count} design(s) for {Safe(() => player.data?.name, "?")}.");
            }
            ...
            catch (Exception ex) { Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_DESIGNPROBE fleet-scan failed: {ex.GetType().Name}: {ex.Message}"); }
        }
        catch (Exception ex)
        {
            Melon<UADVanillaPlusMod>.Logger.Warning($"UADVP_DESIGNPROBE failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void Report(Ship d, string src)
    {
        ...
        Melon<UADVanillaPlusMod>.Logger.Msg(
            $"UADVP_DESIGNPROBE [{src}] name='{name}' erased={B(() => d.isErased)} design={B(() => d.isDesign)} refit={B(() => d.isRefitDesign)} valid={B(() => d.IsValid(false))} hull={Safe(() => d.hull?.data?.name, "?")} type={Safe(() => d.shipType?.name, "?")}");
    }
