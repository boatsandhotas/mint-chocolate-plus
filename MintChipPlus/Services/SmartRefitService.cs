...
namespace MintChipPlus.Services;
...
internal static class SmartRefitService
{
    ...
    private static SmartRefitResult ApplyCore(Ship ship, Player? ownerOverride, SmartRefitOptions options, Ship? powerBaseline = null)
    {
        ...
        try
        {
            ...
            if (options.ComponentOnlyManual && !HasInstalledComponentChanges(componentSummary))
            {
                ...
                Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} {options.Context}: rejected {noOpMessage}.");
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix} {options.Context}: {message}.");
        ...
        catch (Exception ex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} {options.Context}: {message}: {ex.Message}");
        ...
        Stopwatch timer)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} {options.Context}: rejected {message}.");
        return SmartRefitResult.Rejected(message);
    }
        ...
        try
        {
            ...
            if (!canBuild)
            {
                    ...
                    IsRefitDesignForBuildability(ship))
                {
                    ...
                    {
                        Melon<MintChipPlusMod>.Logger.Msg(
                            $"UADMC smart refits ai: buildability-obsolete-allowed design={LogToken(ShipLabel(ship))} reason={LogToken(normalizedReason)} waivedBadParts={reqValidation.WaivedBadParts} source=service-validation.");
                    }
                        ...
                        out string waiverDetails))
                {
                    ...
                    {
                        Melon<MintChipPlusMod>.Logger.Msg(
                            $"UADMC smart refits ai: buildability-waiver-allowed design={LogToken(ShipLabel(ship))} reason={LogToken(normalizedReason)} waivedBadParts={waivedBadParts} source=service-validation details={LogToken(waiverDetails)}.");
                    }
