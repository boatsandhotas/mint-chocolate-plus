...
// deterministic MC attempt. This is deliberately strict: generated new designs
// must pass vanilla buildability without Smart Refit grandfathering waivers.
internal static class SmartAiDesignService
{
    private const string LogPrefix = "UADMC smart AI designs";
    ...
    internal static void FlushPendingTurnSummariesAfterNextTurn()
    {
        ...
        if (summaries.Count == 0)
        {
            ...
            if (LoggedEmptyTurnSummaries.Add(key))
                Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix} summary turn={LogToken(turn)} entries=0");
            return;
        }
        ...
        foreach (SmartAiDesignTurnSummary summary in summaries)
        {
            try
            {
                Melon<MintChipPlusMod>.Logger.Msg(summary.ToLogLine(LogPrefix));
    ...
    internal static void LogSmartRandomShipHookAttached()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: attached GenerateRandomShip smart generator hook.");
    }
