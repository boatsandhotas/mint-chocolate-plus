...
internal static class CampaignFleetMultiChangePortPatch
{
    ...
    private static void RaiseChangePortCap(CampaignFleetWindow window)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC multi change-port cap raise failed; leaving vanilla limit intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogCapRaiseOnce()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC Fleet tab: raised Change Port group cap to {RaisedChangePortCap} so multi-selected ships can be sent to one port at once.");
    }
}
