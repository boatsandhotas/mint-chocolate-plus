...
internal static class ForeignPurchaseBuildRestrictionPatch
{
    ...
    private static void Postfix(Ship design, ref string reason, ref bool __result)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC foreign-purchase build gate failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogOnce(Ship design)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_ALLYBUY blocked NEW build of purchased class (designId={id}); refit still allowed.");
        }
        catch { }
    }
    ...
    private static void LogExemptOnce(Ship design)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_ALLYBUY refit-editor: ALLOWING build of purchased-lineage design (designId={id}) — Ui.isConstructorRefitMode=true (refit commit, not new build).");
        }
        catch { }
    }
}
