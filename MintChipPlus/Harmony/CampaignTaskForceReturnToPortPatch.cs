...
{
    private const string ReturnButtonName = "UADMC_ReturnToOriginPort";
    ...
    private static void Postfix(ShipGroupPopupUI __instance, CampaignController.TaskForce group)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC return-to-port button failed while refreshing the task-force popup. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void ReturnToOrigin(ShipGroupPopupUI popup, CampaignController.TaskForce group)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC return-to-port: ordered task force {group.Id} back to {PortName(origin)}.");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC return-to-port order failed. {ex.GetType().Name}: {ex.Message}");
            MessageBoxUI.Show("Return to Port", "UAD:MC could not order this task force back to port.");
        }
    }
    ...
    private static void ClosePopup(ShipGroupPopupUI popup)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC return-to-port popup close failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void RefreshMapMovementUi()
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC return-to-port map refresh failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
