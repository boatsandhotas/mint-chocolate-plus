...
internal static class CampaignPoliticsCapacityStatusPatch
{
    ...
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC politics capacity status unavailable: CampaignPolitics_ElementUI.GetFinancialInfoOther not found.");
    ...
    private static void LogActiveOnce()
    {
        ...
        loggedActive = true;
        Melon<MintChipPlusMod>.Logger.Msg("UADMC politics capacity status active: appended dockyard and build capacity to financial info rows.");
    }
    ...
    private static void LogFailureOnce(Exception ex)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning(
            $"UADMC politics capacity status failed; leaving vanilla financial info intact. {ex.GetType().Name}: {ex.Message}");
    }
}
