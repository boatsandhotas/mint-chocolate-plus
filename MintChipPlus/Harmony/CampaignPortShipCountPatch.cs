...
internal static class CampaignPortShipCountPatch
{
    ...
    private static bool RefreshPortLabels(MapUI? mapUi)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign port ship counts failed; leaving current port labels intact. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static void LogSummaryOnce(Dictionary<string, int> countsByPort, int decoratedPorts)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC campaign port ship counts: displayed {totalShips} vessels across {countsByPort.Count} campaign ports.");
    }
}
