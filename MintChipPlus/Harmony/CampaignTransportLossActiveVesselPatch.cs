...
{
    private const string LogPrefix = "UADMC TR active area vessels";
    ...
    internal static void Begin(CampaignController? campaign, string source = "TransportLoses window")
    {
        ...
        if (isActive && !loggedDuplicateBegin)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: begin requested while already active; replacing previous context source={source}.");
        }
    ...
    private static void LogNoCacheOnce()
    {
        ...
        loggedNoCache = true;
        Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: campaign area-vessel cache unavailable; leaving vanilla input for this call.");
    }
    ...
    private static void LogTimingTokenOnce(string? msg, string source)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix} timing: source={source} token={token}.");
    }
...
internal static class CampaignTransportLossActiveVesselMeasureTimePatch
{
    ...
    private static bool Prepare()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC TR active area vessels proof: MeasureTime target={(target != null ? "yes" : "no")}.");
        return target != null;
    }
...
internal static class CampaignTransportLossActiveVesselMeasureMethodPatch
{
    ...
    private static bool Prepare()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC TR active area vessels proof: MeasureMethod target={(target != null ? "yes" : "no")}.");
        return target != null;
    }
    ...
    private static void Postfix(string __0)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC TR active area vessels: MeasureMethod hook failed; transport loss input remains vanilla for this callback. {ex.GetType().Name}: {ex.Message}");
        }
    }
}
...
internal static class CampaignTransportLossAreaVesselsInAreaPatch
{
    ...
    private static bool Prepare()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC TR active area vessels proof: VesselsInAreaOfPlayer target={(target != null ? "yes" : "no")}.");
        return target != null;
    }
    ...
    private static void LogActiveOnce()
    {
        ...
        loggedActive = true;
        Melon<MintChipPlusMod>.Logger.Msg("UADMC TR active area vessels: VesselsInAreaOfPlayer filtered during TransportLoses window.");
    }
}
