...
internal static class AllyBuyCountDialog
{
    ...
    internal static void Open(string cls, string nation, int max, AlliedShipPurchase.Quote q, Action<int> onBuyCb)
    {
        ...
        var go = new GameObject("UADMC_AllyBuyCountCanvas");
    ...
    private static void Confirm()
    {
        ...
        int n = count;
        Close();
        try { cb?.Invoke(n); } catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_ALLYBUY count-dialog confirm failed. {ex.GetType().Name}: {ex.Message}"); }
    }
