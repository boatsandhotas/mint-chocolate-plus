...
internal static class ParallelOrder
{
    ...
    internal static void Tick(Ui? ui)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC parallel order failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void BeginTargeting(Ui? ui, string via)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg("UADMC parallel order: select a division first, then click an anchor division.");
            return;
        }
        try { ui.SelectedCommand = Ui.DivisionCommand.Screen; } catch { }
        parallelPending = true;
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC parallel order ({via}): targeting — click the anchor division to run parallel to.");
    }
    ...
    internal static bool TryCapture(Division ordered, Division anchor)
    {
        ...
        if (fp == ap)
        {
            Melon<MintChipPlusMod>.Logger.Msg("UADMC parallel order: anchor must be a different division.");
            return true;
        }

        Links[fp] = new Link { Follower = ordered, Anchor = anchor };
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC parallel order: '{DivName(ordered)}' now running parallel to '{DivName(anchor)}'.");
        return true;
    }
    ...
    internal static void Cancel(Division? d, string reason)
    {
        ...
        try
        {
            if (Links.Remove(d.Pointer))
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC parallel order: '{DivName(d)}' parallel cancelled ({reason}).");
        }
        catch { }
    }
    ...
    private static void DriveLinks()
    {
        ...
        foreach (var kv in Links)
        {
            ...
            if (sample)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC_PARALLELLOG '{DivName(link.Follower)}'->'{DivName(link.Anchor)}' anchorHdg={Hdg(aFwd)} anchorPos=({aPos.x:0},{aPos.z:0}) tgt=({target.x:0},{target.z:0}) folPos=({fp.x:0},{fp.z:0}) dist={dist:0}");
            }
        }
