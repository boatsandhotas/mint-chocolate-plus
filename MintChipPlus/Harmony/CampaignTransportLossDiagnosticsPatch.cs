...
{
    private const string LogPrefix = "UADMC TR sea loss diag";
    ...
    private static void LogFirstMeasureTimeMessage(string? msg)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix} first MeasureTime msg={LogToken(msg)}.");
    }
    ...
    private static void LogMissingFieldOnce(string name)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} unavailable: transport-loss accessor {name} missing.");
    }
    ...
    private static void LogMissingBaselineOnce()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: no SubmarineBattles baseline yet; seeded baseline at TransportLoses.");
    }
    ...
    private static void LogFailureOnce(Exception ex)
    {
        ...
        lastFailure = failure;
        Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} failed; leaving campaign flow unchanged. {ex.GetType().Name}: {ex.Message}");
    }
    ...
    private static class ReportTrLosesPatch
    {
        ...
        {
            MethodBase? target = TargetMethod();
            Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix} proof: ReportTrLoses target={(target != null ? "yes" : "no")}.");
            return target != null;
        }
