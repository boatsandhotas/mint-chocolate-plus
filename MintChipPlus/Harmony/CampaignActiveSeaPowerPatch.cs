...
{
    private const string LogPrefix = "UADMC active sea power";
    ...
    private static bool LogPatchAvailability(string methodName)
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: target missing {methodName}; vanilla area power remains unchanged.");
    ...
    private static void LogActiveOnce()
    {
        ...
        loggedActive = true;
        Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: moving task forces excluded from PP/escort/raid area calculations.");
    }
