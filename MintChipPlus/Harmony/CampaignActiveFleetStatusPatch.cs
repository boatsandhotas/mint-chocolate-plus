...
internal static class CampaignActiveFleetStatusPatch
{
    ...
    internal static void ApplyToInstance(CampaignCountryInfoUI __instance)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC active fleet tooltip setup failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static string DecorateActiveFleetText(Player player, string text)
    {
        ...
        if (summary != lastLoggedSummary)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC active fleet status: displayed {inPort} active vessels in port for {player.Name(false)}.");
        }
