...
internal static class CampaignCanalOpeningPatch
{
    ...
    internal static void ApplyCurrentSetting()
    {
        ...
        if (canals == null)
        {
            Melon<MintChipPlusMod>.Logger.Msg("UADMC canal openings: option stored; campaign canals are not loaded yet.");
            return;
        }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC canal openings: canal state refresh failed. {ex.GetType().Name}: {ex.Message}");
        }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC canal openings: navmesh refresh failed. {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static void LogState(MapCanals canals, string context)
    {
        ...
        foreach (CanalOpening opening in CanalOpenings)
        {
            ...
            if (canal == null)
            {
                Melon<MintChipPlusMod>.Logger.Warning($"UADMC canal openings: {context} could not find {opening.Id}.");
                continue;
            }

            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC canal openings: {context}; {opening.Label} mode {mode}, availability {canal.AvailableFrom}, campaign year {year}, open {canal.IsAvailable}.");
        }
    }
