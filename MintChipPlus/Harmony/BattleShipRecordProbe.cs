...
// managed-reachable (no native bypass — MC already hooks it elsewhere) and carries the ABSOLUTE damage,
...
internal static class BattleShipRecorder
{
    ...
    internal static void Begin()
    {
        ...
        if (Active && ModSettings.BattleRuntimeDiagnosticsEnabled)
            Melon<MintChipPlusMod>.Logger.Msg("UADMC_RECPROBE battle started — recording per-ship damage/kills.");
    }
    ...
    internal static void EndBattleLog()
    {
        try
        {
            ...
            {
                var log = Melon<MintChipPlusMod>.Logger;
                foreach (IntPtr vp in sunk)
                {
                    ...
                    log.Msg($"UADMC_RECPROBE SUNK {victim}: sankBy={sank} wreckedBy={wrecked}({wreckDmg:0}) totalReceived={recv:0}");
                }
                ...
                foreach (var ad in SortedDesc(DealtTotal))
                {
                    ...
                    log.Msg($"UADMC_RECPROBE DEALER {NameOf(ad.Key)}: dealt={ad.Value:0} kills={kills}");
                }

                log.Msg($"UADMC_RECPROBE battle ended — {sunk.Count} ship(s) sunk, {DealtTotal.Count} attacker(s) tracked.");
            }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC_RECPROBE end-log failed: {ex.GetType().Name}: {ex.Message}");
        }
        finally
        {
            Clear();
    ...
    private static void PersistPlayerRecords(HashSet<IntPtr> sunk)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC ship records persist failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void Postfix(Ship victim, Part from, float damage)
    {
        ...
        try
        {
            ...
            if (ModSettings.BattleRuntimeDiagnosticsEnabled && BattleShipRecorder.HitDiag < 20)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC_RECPROBE HIT#{BattleShipRecorder.HitDiag} from={an} -> {vn} dmg={damage:0} dead={dead}");
            }
