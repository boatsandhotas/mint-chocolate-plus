...
internal static class BattleRuntimeAccuracyDiagnostics
{
    ...
    internal static void EnterBattle(string context)
    {
        ...
        LogOnce("active:" + context, $"UADMC battle-runtime diagnostics active: context={context} sampleInterval={SampleIntervalSeconds:0.#}s.");
    }
    ...
    internal static void UpdateBattle()
    {
        ...
        catch (Exception ex)
        {
            ...
            LogOnce("sample-failed:" + ex.GetType().Name,
                $"UADMC battle-runtime diagnostics sample skipped. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void PrintSummary(string context)
    {
        ...
        try
        {
            ...
            {
                SideRuntimeAggregate aggregate = SideAggregates[side];
                Melon<MintChipPlusMod>.Logger.Msg(aggregate.Format(sessionSamples, readFailures, sessionSignature, context));
                ...
                Melon<MintChipPlusMod>.Logger.Msg(weaponAggregate.Format());
                ...
                Melon<MintChipPlusMod>.Logger.Msg(damageAggregate.Format());
            }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle-runtime diagnostics summary failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static bool TryReadInstabilityDamage(Ship ship, out float value)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (!instabilityDamageReadFailureLogged)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC battle-runtime diagnostics: Ship.instabilityDamage read failed via {instabilityDamageOffsetSource}; damage instability summaries will show n/a. {ex.GetType().Name}: {ex.Message}");
            }
        }
    ...
    private static void LogInstabilityDamageResolution(bool available, int offset, string source)
    {
        ...
        if (available)
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC battle-runtime diagnostics: resolved Ship.instabilityDamage offset=0x{offset:X} via {source}.");
        }
        else
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                "UADMC battle-runtime diagnostics: Ship.instabilityDamage field unavailable; damage instability summaries will show n/a.");
        }
    }
