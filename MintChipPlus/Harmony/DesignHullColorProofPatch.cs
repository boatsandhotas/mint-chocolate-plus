...
// unless the player enables Experimental Nation Ship Paints in the UAD:MC menu.
...
internal static class DesignHullColorProofPatch
{
    ...
    private const string GeneratedMarker = "_uadmc_";
    ...
    {
        if (EnablePaintLogs)
            Melon<MintChipPlusMod>.Logger.Msg(msg);
    }
    ...
    internal static void RefreshNationPaintSettingsCache(string context)
    {
        ...
        if (NationPaintSettingsLogCount++ < 3)
            PaintLog($"UADMC ship paint proof: refreshed Nation Ship Paints settings during {context}.");
    }
    ...
    internal static void ApplyCurrentSetting()
    {
        ...
        EnsureNationPaintSchemeCache();
        PaintLog("UADMC ship paint proof: Experimental Nation Ship Paints enabled.");
        if (GameManager.IsBattle)
    ...
    internal static void ApplyNationPaintSettingsChange(string context)
    {
        ...
        if (GameManager.IsBattle || GameManager.IsConstructor)
            ...
            PaintLog($"UADMC ship paint proof: stored Nation Ship Paints change during {context}; no active ship scene repaint needed.");
    }
    ...
    internal static void ResetScenePaintCache(string context)
    {
        ...
        {
            PaintLog(
                $"UADMC ship paint proof: reset scene paint cache during {context}; restoredLoadedRenderers={restoredLoadedRenderers}, restoredTrackedRenderers={restoredTrackedRenderers}, materialSets={materialSets}, generatedMaterials={materials}, destroyedGeneratedObjects={generatedObjects}.");
        }
    }
    ...
    private static int RestoreLoadedPartMaterials(string context)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint proof failed to restore loaded parts during {context}. {ex.GetType().Name}: {ex.Message}");
            return 0;
        }
    }
    ...
    private static int DestroyGeneratedPaintObjects(string context)
    {
        ...
        {
            PaintLog(
                $"UADMC ship paint proof: destroyed {destroyed} generated paint object(s) during {context}.");
        }
    ...
    private static bool DestroyUnityObject(UnityEngine.Object? obj)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC ship paint proof failed to destroy generated paint object '{obj.name ?? "<object>"}'. {ex.GetType().Name}: {ex.Message}");
            }
    ...
    internal static void ScheduleBattleRepaintRetries(string context, bool repaintImmediately)
    {
        ...
        if (BattleRepaintScheduled && ShouldContinueBattleRepaintRetry(BattleRepaintScheduledGeneration))
        {
            ...
            {
                PaintLog(
                    $"UADMC ship paint proof: coalesced battle repaint request during {context}; pendingGeneration={BattleRepaintScheduledGeneration}.");
            }
    ...
    internal static void RepaintAllLoadedParts(string context)
    {
        ...
        try
        {
            ...
            {
                PaintLog(
                    $"UADMC ship paint proof: repainted loaded parts during {context}; parts={parts.Length}, candidates={repaintCandidates}, overThreshold={candidatesOverThreshold}.");
            }
            else if (candidatesOverThreshold > 0 && BattleRepaintThresholdLogCount++ < 4)
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC ship paint proof: repaint candidate count exceeded threshold by {candidatesOverThreshold} during {context}; threshold={BattleRepaintCandidateWarningThreshold}.");
            }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint proof failed to repaint loaded parts during {context}. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void TryApplyProofColor(Part part, string context, bool force)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC ship paint proof failed during {context}. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void EnsureNationPaintSchemeCache()
    {
        ...
        foreach (NationPaintDefinition definition in NationPaintDefinitions)
        {
            ...
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC ship paint proof: invalid Nation Ship Paints string for {definition.Label}; using built-in scheme. {error}");
            }
        }
    ...
    private static PaintProfile CustomProfile(Color32 color, PaintProfile fallback, string nationKey, string area)
        => new(
            ...
            $"_uadmc_custom_{nationKey}_{area}_{HexString(color).TrimStart('#').ToLowerInvariant()}");
    ...
    internal static void QueueCampaignBattleCountryMap(CampaignBattle? battle, string context)
    {
        ...
        {
            PaintLog(
                $"UADMC ship paint proof: queued campaign battle paint country map during {context} for battle {SafeCampaignBattleId(battle)}.");
        }
    }
    ...
    internal static void RememberBattleStateCampaignCountries(string context)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint proof: campaign battle country capture failed during {context}. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void RememberCurrentCampaignBattleCountries(CampaignBattle? battle, string context)
    {
        ...
        if (BattleCountryMapLogCount++ < 4)
        {
            ...
            PaintLog(
                $"UADMC ship paint proof: remembered campaign battle paint countries during {context} for {mapped} ship(s): {countries}.");
        }
    }
    ...
    internal static void RememberRealBattleCountries(GameManager.RealBattleSave? save)
    {
        ...
        if (BattleCountryByShipId.Count > 0 && BattleCountryMapLogCount++ < 4)
        {
            string countries = string.Join(", ", BattleCountryByShipId.Values.Distinct(StringComparer.OrdinalIgnoreCase));
            PaintLog(
                $"UADMC ship paint proof: remembered battle paint countries for {BattleCountryByShipId.Count} ship(s): {countries}.");
        }
    }
    ...
    internal static void RememberDamageVisualPolicy(Part? part, Part.Damage damageState)
    {
        ...
        {
            PaintLog(
                $"UADMC ship paint proof: yielding paint to vanilla damage visuals on {SafePartName(part.data)}; damage={damageState}.");
        }
    }
    ...
    private static IEnumerable<Renderer> LodRenderers(Part part, HashSet<int> seen)
    {
        ...
        {
            PaintLog(
                $"UADMC ship paint proof: included {lodRenderers.Count} extra {AreaLabel(paintArea ?? PaintArea.Superstructure)} LOD renderer(s) on {SafePartName(part.data)}.");
        }
    ...
    private static void LogMissedGunChildRenderers(Part part, HashSet<int> visualRendererIds)
    {
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            PaintLog(
                $"UADMC ship paint proof: gun child renderer outside visualRenderers on {SafePartName(part.data)}; " +
    ...
    private static void ScanShipForOrphanRenderers(Part triggeringPart)
    {
        ...
        try
        {
            ...
            foreach (Renderer renderer in renderers)
            {
                ...
                orphanCount++;
                PaintLog(
                    $"UADMC ship paint ORPHAN renderer (no Part parent) on '{shipName}'; path={path}; activeSelf={renderer.gameObject.activeSelf}; activeInHierarchy={renderer.gameObject.activeInHierarchy}; materials={materialNames}.");
            ...
            PaintLog(
                $"UADMC ship paint ship scan summary: ship='{shipName}'; parts={allParts.Length}; totalRenderers={renderers.Length}; orphanRenderersFound={orphanCount}.");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint ship scan failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogMissedChildRenderers(Part part, HashSet<int> seenRendererIds)
    {
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            PaintLog(
                $"UADMC ship paint proof: missed child renderer (part={areaLabel}) on {SafePartName(part.data)}; " +
                $"renderer={rendererPath}; parent={parentPath}; activeSelf={renderer.gameObject.activeSelf}; " +
    ...
    private static int RestoreTrackedRenderers(string context)
    {
        ...
        foreach (RendererOriginalMaterialSet originalSet in OriginalMaterialsByPaintedRenderer.Values.ToArray())
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Melon<MintChipPlusMod>.Logger.Warning(
                        $"UADMC ship paint proof failed to restore a tracked renderer during {context}. {ex.GetType().Name}: {ex.Message}");
                }
            }
        }
    ...
    private static void LogPropertyBlockFailure(Material source, PaintArea paintArea, Exception ex)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning(
            $"UADMC ship paint proof failed to apply property block for '{source.name ?? "<material>"}' ({AreaLabel(paintArea)}). {ex.GetType().Name}: {ex.Message}");
    }
    ...
    private static Material? GetOrCreatePaintMaterial(Material material, PaintArea paintArea, PaintProfile profile, bool trustOuterClassifier = false)
    {
        ...
        catch (Exception ex)
        {
            FailedMaterialCopies.Add(key);
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint proof failed to clone material '{source.name}' ({AreaLabel(paintArea)}). {ex.GetType().Name}: {ex.Message}");
            return null;
        }
    }
    ...
    private static int RestoreOriginalMaterials(Part part, string context, bool logDetails = true)
    {
        try
        {
            ...
            if (rendererCount > 0)
            {
                ...
                {
                    PaintLog(
                        $"UADMC ship paint proof: restored {materialCount} generated material(s) on {SafePartName(part.data)} before {context}; renderers={rendererCount}.");
                }
            }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint proof failed to restore generated materials before {context}. {ex.GetType().Name}: {ex.Message}");
            return 0;
        }
    }
    ...
    private static void LogRestoredBrokenMaterial(Material brokenMaterial, Material originalMaterial)
    {
        ...
        PaintLog(
            $"UADMC ship paint proof: restored broken generated material '{brokenMaterial.name ?? "<material>"}' to '{originalMaterial.name ?? "<material>"}'.");
    }
    ...
    private static Texture? GetOrCreatePaintTexture(Texture? source, PaintArea paintArea, PaintProfile profile)
    {
        ...
        try
        {
            ...
            {
                PaintLog(
                    $"UADMC ship paint proof: generated {AreaLabel(paintArea)} texture '{copy.name}' from '{source.name}' ({source.width}x{source.height}).");
            }
        ...
        catch (Exception ex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship paint proof failed for '{source.name}' ({AreaLabel(paintArea)}). {ex.GetType().Name}: {ex.Message}");
            return null;
        }
        finally
        {
    ...
    private static void LogUnclassifiedPartSample(Part part)
    {
        ...
        try
        {
            ...
            PaintLog(
                $"UADMC ship paint unclassified part sample #{LoggedUnclassifiedPartSamples.Count}: part='{partName}'; ship='{shipName}'.");
        }
        catch
        {
            // Diagnostics only.
    ...
    private static void LogUnmatchedMaterial(Material material, bool hasPrimary, PaintArea primaryArea)
    {
        ...
        try
        {
            ...
            PaintLog(
                $"UADMC ship paint UNMATCHED sample #{LoggedUnmatchedMaterialSamples.Count}: primary={primaryDescriptor}; material='{sourceName}'; textures='{textures}'.");
        }
        catch
        {
            // Diagnostics only.
    ...
    private static void LogEncounteredMaterial(Material source, bool hasPrimary, PaintArea primaryArea)
    {
        ...
        try
        {
            ...
            string primaryDescriptor = hasPrimary ? primaryArea.ToString() : "<unclassified>";
            PaintLog(
                $"UADMC ship paint MATERIAL SEEN #{EncounteredMaterialLogCount}: partArea={primaryDescriptor}; material='{sourceName}'; textures='{textures}'.");
        }
        catch
        {
            // Diagnostics only.
    ...
    private static void LogSilentlySkippedMaterial(Material source, PaintArea paintArea, string reason)
    {
        ...
        try
        {
            ...
            PaintLog(
                $"UADMC ship paint SILENT SKIP #{SilentlySkippedMaterialLogCount}: area={paintArea}; reason={reason}; material='{sourceName}'; textures='{textures}'.");
        }
        catch
        {
            // Diagnostics only.
    ...
    private static void LogFirstApplication(Part part, PaintArea paintArea, ShipPaintScheme scheme, string context, int rendererCount, int changedMaterialCount, int skippedMaterialCount)
    {
        ...
        if (areaLogCount >= MaxApplicationLogsPerArea)
        {
            ...
            {
                PaintLog(
                    $"UADMC ship paint proof: further {AreaLabel(paintArea)} application logs suppressed.");
            }
        ...
        string partName = SafePartName(part.data);
        PaintLog(
            $"UADMC ship paint proof: tinted {AreaLabel(paintArea)} using {scheme.Id} for {shipName} / {partName} during {context}; renderers={rendererCount}, paintedMaterials={changedMaterialCount}, skippedMaterials={skippedMaterialCount}.");
    ...
    private static void LogMaterialSamples(Part part, PaintArea paintArea)
    {
        ...
        foreach (Renderer renderer in HullRenderers(part))
        {
            ...
            foreach (Material material in materials)
            {
                ...
                PaintLog(
                    $"UADMC ship paint sample ({AreaLabel(paintArea)}): {verdict}; renderer='{SafeObjectName(renderer.gameObject)}'; material='{material.name ?? "<material>"}'; textures='{MaterialTextureNames(material)}'.");
