...
internal static class DesignSuperstructureRefitAvailabilityPatch
{
        ...
        ref bool __result)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC superstructure compatibility fallback failed for {SafePartName(part)}: {e.GetType().Name}: {e.Message}");
        }
    }
    ...
    internal static void RebuildHullProxyIndex(UadGameData? gameData)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC superstructure compatibility indexed {HullProxiesByToken.Count} hull-family proxy token(s) from {hullCount} hull(s); unlock years for {HullUnlockYearByName.Count} part(s).");
    }
        ...
        NeedRelaxationPlan plan)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC superstructure compatibility proxy availability failed for {SafePartName(part)}: {e.GetType().Name}: {e.Message}");
            return false;
        }
        finally
        {
    ...
    private static void EnsureHullProxyIndex()
    {
        ...
        {
            hullProxyIndexBuilt = true;
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC superstructure compatibility could not build hull proxy index: {e.GetType().Name}: {e.Message}");
        }
    }
    ...
    private static void LogGroupOnce(string partGroup, string targetGroup)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC superstructure compatibility considering proxy need gates for {partGroup} part(s) on {targetGroup} hull(s).");
    }
    ...
    private static void LogAllowedOverride(PartData part, ShipType shipType, PartData? currentHull, string partGroup, string targetGroup, NeedRelaxationPlan plan)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            "UADMC superstructure compatibility allowed " +
        ...
        string? proxySummary)
    {
        ...
        deniedLogCount++;
        Melon<MintChipPlusMod>.Logger.Msg(
            "UADMC superstructure compatibility denied " +
