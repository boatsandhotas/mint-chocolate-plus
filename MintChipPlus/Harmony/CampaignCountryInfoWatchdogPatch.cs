...
// MC markers and only reapplies the heavier calculations when vanilla has just
...
internal static class CampaignCountryInfoWatchdogPatch
{
    ...
    internal static void LateUpdatePostfix(Ui __instance)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign watchdog refresh failed; leaving current text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogMarkerStateOnce(CampaignCountryInfoUI countryInfo, int applied)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC campaign watchdog markers instance/active/maint/tr/tech: {state}.");
    }
        ...
        string after)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC campaign watchdog maintenance repair: {state}.");
    }
