...
// summary even though they are still not active ships. MC keeps the panel compact
...
internal static class CampaignConstructionStatusPatch
{
    ...
    private const string MaintenanceLineName = "UADMC_MaintenanceLine";
    ...
    internal static void ApplyToInstance(CampaignCountryInfoUI __instance)
    {
            ...
            if (TryBuildCounts(player, out ConstructionCounts counts))
            {
                ...
                if (summary != lastLoggedSummary)
                {
                    ...
                    Melon<MintChipPlusMod>.Logger.Msg(
                        $"UADMC campaign construction: displayed {counts.TotalDisplay} ships as {counts.TypeDisplay} in {stopwatch.ElapsedMilliseconds} ms.");
                }
            }
            else
            {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign construction status patch failed; leaving vanilla text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static bool TryBuildCounts(Player player, out ConstructionCounts counts)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign construction count failed; falling back to vanilla. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static void LogApplySkipOnce(string reason)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC campaign construction partial apply: {reason}; maintenance status was still attempted.");
    }
    ...
    private static void UpdateMaintenanceStatus(CampaignCountryInfoUI ui, Player player, Stopwatch stopwatch)
    {
        ...
        if (summary != lastLoggedMaintenanceSummary)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC campaign maintenance status: displayed {summary} in {stopwatch.ElapsedMilliseconds} ms.");
        }
