...
internal static class CampaignFleetWindowDesignViewerPatch
{
    ...
    private const string DesignPowerColumnName = "UADMC_DesignPower";
    private const string DesignPowerHeaderName = "UADMC_DesignPowerHeader";
    ...
    private static void EnsureDesignViewerToolbar(CampaignFleetWindow window)
    {
        ...
        if (designViewerToolbar == null)
        {
            designViewerToolbar = new GameObject("UADMC_DesignViewerToolbar");
    ...
    private static void RebuildDesignViewerToolbarIfNeeded()
    {
        ...
        foreach (Player player in players)
        {
            ...
            GameObject flagButton = new("UADMC_DesignViewerFlag");
            flagButton.AddComponent<RectTransform>();
            flagButton.transform.SetParent(designViewerToolbar.transform, false);
            flagButton.name = $"UADMC_DesignViewerFlag_{player.data?.name ?? player.Name(false)}";
            ...
            GameObject flagImageObject = new("UADMC_FlagImage");
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC design viewer: rebuilt nation selector for {players.Count} player(s); campaign={RuntimeObjectKey(CampaignController.Instance)}.");
    }
    ...
    private static void OnDesignViewerFlagClicked(Player player)
    {
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC design viewer: flag clicked {PlayerLabel(GetCurrentDesignViewerPlayer())} -> {PlayerLabel(player)}; campaign={RuntimeObjectKey(CampaignController.Instance)}.");
    ...
    private static void SetDesignViewerPlayer(CampaignFleetWindow window, Player player)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC design viewer: ignored stale nation selector target {PlayerLabel(player)}; rebuilding selector.");
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC design viewer: resolved stale nation selector target {PlayerLabel(player)} -> {PlayerLabel(resolvedPlayer)}; rebuilding selector.");
            designViewerToolbarSignature = string.Empty;
            RebuildDesignViewerToolbarIfNeeded();
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"Design viewer country switch failed; restoring player designs. {ex.GetType().Name}: {ex.Message}");
    ...
    private static string DesignPowerDisplayText(Ship design)
    {
        ...
        try
        {
            ...
            float displayPower = ShipEffectivePowerCalculator.ComputeDisplayPower(design, power.AdjustedPower);
            text = ShipEffectivePowerCalculator.FormatCompactPower(displayPower);
        }
        catch
        {
            text = "-";
    ...
    private static void LogRefitDesignNameCleanupOnce()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg("Design viewer: normalized refit design row names.");
    }
    ...
    private static void LogDesignPowerColumnOnce()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg("UADMC design viewer: added power estimate column.");
    }
    ...
    {
        // Vanilla only shows the current player's design list. MC also includes
    ...
    private static void SetForeignDesignButtonsInteractable(CampaignFleetWindow window, bool interactable)
    {
        ...
        {
            if (child == designViewerToolbar || child.name.StartsWith("UADMC_DesignViewer"))
                continue;
    ...
    {
        // Vanilla's design action listeners read selectedElements[0]. MC can
    ...
    // appears (named "UADMC_DesignViewer*" so it survives SetForeignDesignButtonsInteractable's disable
    ...
    private const string DesignViewerPurchaseButtonName = "UADMC_DesignViewerPurchase";
    ...
    private static void EnsureDesignViewerPurchaseButton(CampaignFleetWindow window)
    {
        ...
        clone.name = DesignViewerPurchaseButtonName; // MUST start with "UADMC_DesignViewer" (exemption in SetForeignDesignButtonsInteractable)
    ...
    private static void UpdateDesignViewerPurchaseButton(CampaignFleetWindow window)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC design viewer purchase button update failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void OnDesignViewerPurchaseClicked(CampaignFleetWindow window)
    {
        try
        {
            ...
            AllyBuyCountDialog.Open(cls, nation, maxCount, q, new System.Action<int>(n =>
            {
                ...
                {
                    Melon<MintChipPlusMod>.Logger.Warning($"UADMC purchase confirm failed. {ex.GetType().Name}: {ex.Message}");
                }
            }));
        }
        catch (System.Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC design viewer purchase click failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void InstallDesignViewButtonHandler(CampaignFleetWindow window, bool allowActions, Ship capturedTarget = null)
    {
        ...
        window.DesignView.onClick.AddListener(new System.Action(() =>
        {
            ...
            {
                if (!allowActions)
                    Melon<MintChipPlusMod>.Logger.Msg("UADMC design viewer: foreign read-only design view ignored; selected design is unavailable.");
                return;
            }
            ...
            if (foreignView)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC design viewer: opening foreign design read-only nation={LogToken(PlayerLabel(owner))} type={ShipClassLabel(target)} name={LogToken(SafeShipName(target))} year={ShipDesignYear(target)} allowEdit=false.");
            }
            ...
            catch (Exception ex)
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC design viewer: read-only design view failed design={DesignLogName(target)}. {ex.GetType().Name}: {ex.Message}");
            }
        }));
    }
        ...
        string context)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC DesignViewer stale vanilla preview guard failed during {context}. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
        ...
        string context)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            "UADMC DesignViewer stale vanilla preview map cleared before refresh: " +
    ...
    private static Il2CppSystem.Collections.Generic.List<Ship> BuildUiBackedDesignList(CampaignFleetWindow window, Il2CppSystem.Collections.Generic.List<Ship> requestedDesigns, string context)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC DesignViewer list mismatch during {context}: " +
    ...
    private static bool TrySetDesignImageAndInfo(CampaignFleetWindow window, Il2CppSystem.Collections.Generic.List<Ship> requestedDesigns, Ship nextShip, bool isDesign, string context)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC DesignViewer stale selection during {context}: selected={DesignLogName(safeNextShip)} is not present in designUiByShip; clearing selection.");
            safeNextShip = null;
            SelectedViewedDesign = null;
        }
        ...
        catch (Exception ex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC DesignViewer SetDesignImageAndInfo failed during {context}: " +
    ...
    private static void RefreshViewedDesigns(CampaignFleetWindow window, bool allowActions)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"Design viewer refresh failed; restoring vanilla player designs. {ex.GetType().Name}: {ex.Message}");
            if (allowActions)
            {
    ...
    private static void SelectViewedDesign(CampaignFleetWindow window, Il2CppSystem.Collections.Generic.List<Ship> designs, Ship ship, FleetWindow_ShipElementUI ui, bool allowActions)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC DesignViewer ignored stale design click: {DesignLogName(ship)} is no longer present in designUiByShip.");
            SelectedViewedDesign = null;
            return;
        }
    ...
    private static void RebuildDesignRefitButton(CampaignFleetWindow window, bool allowActions)
    {
        ...
        {
            window.DesignRefit.interactable = false;
            Melon<MintChipPlusMod>.Logger.Warning($"Design refit button rebuild failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void PostfixRefresh(CampaignFleetWindow __instance, bool isDesign)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"Fleet window design viewer patch failed; leaving vanilla UI intact. {ex.GetType().Name}: {ex.Message}");
            try
            {
                HideDesignViewer();
            }
