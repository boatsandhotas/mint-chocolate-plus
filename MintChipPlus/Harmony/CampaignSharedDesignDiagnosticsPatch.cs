...
internal static class CampaignSharedDesignDiagnosticsPatch
{
    ...
    private static void LogDebug(string message)
        => Melon<MintChipPlusMod>.Logger.Msg($"{DebugLogPrefix} {message}");
...
internal static class CampaignSharedDesignTryTakeDiagnosticsPatch
{
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("[AI SharedDesign] TryTakeSharedDesign target not found; shared-design attempt diagnostics disabled.");
...
internal static class CampaignSharedDesignGapNextTurnCompletionPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("[AI SharedDesign] NextTurn MoveNext target not found; deferred gap summary flush will use fallback timing.");
...
internal static class CampaignSharedDesignGetSharedDesignDiagnosticsPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("[AI SharedDesign] GetSharedDesign target not found; shared-design candidate diagnostics disabled.");
