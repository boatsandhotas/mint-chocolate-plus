...
{
    private const string LogPrefix = "UADMC shared-design year input";
    private const string InputName = "UADMC_SharedDesignYearInput";
    private const string FlagGridName = "UADMC_SharedDesignFlagGrid";
    ...
    internal static void RefreshConstructorUi()
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: UI refresh failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static void CaptureCampaignYear(CampaignController? campaign, string source)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: captured campaign year={year} source={source}.");
    }
    ...
    internal static int PreserveCurrentYearForNewDesign(int vanillaYear, bool log)
    {
        ...
        try
        {
            ...
            if (log)
                Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: preserved shared-design year={preservedYear} for new design.");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
    ...
    private static GameObject EnsureYearInput(GameObject nationYear, GameObject template)
    {
        ...
        {
            loggedAttached = true;
            Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: attached direct year entry field ({MinYear}-{MaxYear}).");
        }
    ...
    private static void EnsureFlagGrid(GameObject nationYear, GameObject? chooseCountry)
    {
        ...
        if (!loggedFlagGridAttached)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: attached nation flag grid nations={nations.Count} columns={FlagGridColumns}.");
        }
    }
    ...
    {
        GameObject buttonObject = new($"UADMC_SharedDesignFlag_{NationKey(nation)}");
    ...
    private static void SelectSharedDesignNation(PlayerData nation)
    {
        ...
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: refreshed shared designs for nation={NationKey(nation)} year={year}.");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: failed to refresh nation={NationKey(nation)} year={year}: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void RefreshFlagGridSelection()
    {
        ...
        for (int i = 0; i < flagGridObject.transform.childCount; i++)
        {
            ...
            string key = child.gameObject.name.Replace("UADMC_SharedDesignFlag_", string.Empty, StringComparison.Ordinal);
    ...
    private static void ApplyEnteredYear(string value)
    {
        ...
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: refreshed shared designs for year={parsedYear}.");
        }
        catch (Exception ex)
        {
            SyncYearText(currentYear);
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: failed to refresh year={parsedYear}: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogMissingFlagGridSourceOnce()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: skipped nation flag grid; no shared-design nations found.");
    }
