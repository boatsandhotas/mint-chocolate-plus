...
internal static class CampaignResearchDiffusionPatch
{
    ...
    internal static void GetResearchSpeedPostfix(Player player, Technology tech, ref float __result)
    {
        ...
        catch (Exception ex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC research diffusion skipped after an unexpected error. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void FlushTurnSummary()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC technology spread turn summary: {ModSettings.TechnologySpreadModeText(mode)}{CampaignDateText()}; {TurnSummariesByPlayer.Count} nations received diffusion boosts.");
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC technology spread turn summary: {summary.LogText}.");
        }
