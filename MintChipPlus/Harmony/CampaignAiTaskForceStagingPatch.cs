...
{
    private const string LogPrefix = "UADMC ai taskforce staging";
...
internal static class CampaignAiTaskForceStagingMergePatch
{
    ...
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: MergeShipGroups target not found; merge observation disabled.");
...
internal static class CampaignAiTaskForceStagingRemovePatch
{
    ...
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: RemoveTaskForce target not found; remove observation disabled.");
...
internal static class CampaignAiTaskForceStagingSummaryNextTurnPatch
{
    ...
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: NextTurn MoveNext target not found; turn summary flush will be skipped.");
