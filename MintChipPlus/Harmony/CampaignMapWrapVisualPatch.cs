...
internal static class CampaignMapWrapVisualPatch
{
    ...
    internal static void ApplyCurrentSetting()
    {
        ...
        ClearWrapObjects();
        Melon<MintChipPlusMod>.Logger.Msg("UADMC map wrap: disabled and restored vanilla horizontal borders.");
    }
    ...
    internal static void PrepareMoveWindowShowMapToMap(CampaignController.TaskForce group, Vector3 from, ref Vector3 to, PathResult result)
    {
        ...
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC map wrap diag move {LastMapClickId}.{MovementDiagnosticsForLastClick}: " +
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC map wrap move-window show diagnostic failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void CreateWrapCopies(CampaignMap map)
    {
        ...
        if (copySpacing <= 0f)
        {
            Melon<MintChipPlusMod>.Logger.Warning("UADMC map wrap skipped: map renderer bounds were not ready.");
            return;
        }
        ...
        if (mapMesh == null)
        {
            Melon<MintChipPlusMod>.Logger.Warning("UADMC map wrap skipped: map mesh was not ready.");
            return;
        }
        ...
        if (mapTexture == null)
        {
            Melon<MintChipPlusMod>.Logger.Warning("UADMC map wrap skipped: map texture was not ready.");
            return;
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC map wrap: enabled seamless visual rotation at +/-{copySpacing:0.###} world x-units.");
    }
    ...
    private static void CreateMapCopy(Renderer sourceRenderer, GameObject sourceObject, Mesh sourceMesh, Texture sourceTexture, int lap, string sideName)
    {
        ...
        GameObject copy = new($"UADMC_WrapMap_{sideName}");
    ...
    private static void LogMapMaterialCopiesOnce(int materialCount)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC map wrap: copying wrapped map surfaces with {materialCount} source material(s).");
    }
    ...
    private static void CreateNativeGridCopies(CampaignMap map)
    {
        ...
        if (grid == null)
        {
            Melon<MintChipPlusMod>.Logger.Warning("UADMC map wrap: native grid not found.");
            return;
        }

        CreateOffsetClone(grid.gameObject, -1, "UADMC_WrapNativeGrid_NegativeX", 0f, null);
        CreateOffsetClone(grid.gameObject, 1, "UADMC_WrapNativeGrid_PositiveX", 0f, null);
    }
    ...
    private static void CreateStaticVisualLayerCopies(Transform? sourceRoot, string layerName)
    {
        if (sourceRoot == null)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC map wrap: {layerName} root not found.");
            return;
        }

        CreateOffsetClone(sourceRoot.gameObject, -1, $"UADMC_WrapStatic_NegativeX_{layerName}", LabelLift, LabelRenderQueue);
        CreateOffsetClone(sourceRoot.gameObject, 1, $"UADMC_WrapStatic_PositiveX_{layerName}", LabelLift, LabelRenderQueue);
    }
    ...
    internal static void RefreshVisibleSideMaps(Cam? cam)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC map wrap side-map visibility failed. {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static DynamicMarkerCopySet? GetOrCreateDynamicMarkerCopies(CampaignMapElement source)
    {
        ...
        if (!LoggedDynamicMarkerCopies)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg("UADMC map wrap: syncing wrapped dynamic marker copies.");
        }
    ...
    private static GameObject? CreateDynamicMarkerCopy(CampaignMapElement source, string sideName, DynamicMarkerKind kind, float xOffset)
    {
        ...
        copy.name = $"UADMC_WrapDynamic_{sideName}_{kind}_{source.gameObject.name}";
        copy.SetActive(true);
    ...
    {
        return gameObject != null && gameObject.name.StartsWith("UADMC_WrapDynamic_", StringComparison.Ordinal);
    }
    ...
    private static bool ExtendWrappedMovePathToDestination(Vector3 destination, PathResult result, out Vector3 extendedFrom, out Vector3 extendedTo)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC map wrap path extension failed. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static void LogMapClickDiagnostic(Vector3 rawPosition, Vector3 normalizedPosition, Vector3 desiredPosition, Vector3 effectivePosition)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC map wrap diag click {LastMapClickId}: wrapped={LastMapClickWasWrapped}, " +
    ...
    private static RouteCopySet? GetOrCreateRouteCopies(Route source)
    {
        ...
        if (!LoggedRouteCopies)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg("UADMC map wrap: syncing wrapped route copies.");
        }
    ...
    private static Route? CreateRouteCopy(Route source, string sideName)
    {
        ...
        copyObject.name = $"UADMC_WrapRoute_{sideName}_{source.gameObject.name}";
    ...
    private static void TryCreateDelayedCountryOverlayCopies(CampaignMap map)
    {
        ...
        RefreshVisibleSideMaps(Cam.Instance);
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC map wrap: copied {createdCopies} country overlay renderers.");
    }
    ...
    private static bool CreateCountryOverlayCopy(MeshRenderer sourceRenderer, int lap, string sideName, string countryName, int regionIndex)
    {
        ...
        GameObject copy = new($"UADMC_WrapOverlay_{sideName}_{safeCountryName}_{regionIndex}");
    ...
    private static void LogCountryBorderDiagnostics(CampaignMap map)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg("UADMC map wrap border diag: manager=no");
            return;
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC map wrap border diag: manager=yes countries={countryCount} overlayMeshes={overlayRendererCount} dummyRenderers={dummyRendererCount} highlightRoots={highlightRootCount} highlightRenderers={highlightRendererCount} wrapOverlayChildren={highlightWrapRendererCount} mapParentRenderers={mapParentRendererCount} namedCandidates={namedCandidateCount}; namedSamples={namedSampleText}; otherSamples={otherSampleText}");
    ...
    private static void LogSceneBorderDiagnostics(CampaignMap map)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC map wrap scene border diag: renderers={totalRenderers} visible={visibleRenderers} excludedMap={excludedMapRenderers} excludedUADMC={excludedVpRenderers} lineCandidates={lineRendererCount} keywordCandidates={keywordCandidateCount}; lineSamples={lineSampleText}; keywordSamples={keywordSampleText}");
    }
    ...
    private static void LogGenericMeshDiagnostics(CampaignMap map)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC map wrap mesh diag: meshRenderers={totalMeshRenderers} visible={visibleMeshRenderers} mapMeshes={mapMeshRenderers} mapLabelText={mapLabelRenderers} excludedUADMC={excludedVpRenderers} nonTextSceneMeshes={nonTextSceneRenderers}; mapSamples={mapSampleText}; sceneSamples={sceneSampleText}");
    }
    ...
    private static bool IsUadVpGeneratedTransform(Transform? transform)
    {
        ...
        {
            string name = transform.name ?? string.Empty;
            if (name.StartsWith("UADMC_", StringComparison.Ordinal))
                return true;
    ...
    private static void SetHorizontalBorderRenderers(Transform? mapParent, bool enabled)
    {
        ...
        foreach (string borderName in borderNames)
        {
            ...
            if (border == null)
            {
                Melon<MintChipPlusMod>.Logger.Warning($"UADMC map wrap: border not found: {borderName}");
                continue;
            }
    ...
    private static Shader FindFirstShader(params string[] shaderNames)
    {
        ...
        throw new InvalidOperationException("No usable shader found for UADMC map wrap material.");
    }
