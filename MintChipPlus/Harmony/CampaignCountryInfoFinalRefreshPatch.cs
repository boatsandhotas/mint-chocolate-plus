...
// after individual data updates. Reapplying MC's campaign UI decorations at
    ...
    internal static void RefreshPostfix(Ui __instance)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign final UI refresh failed; leaving vanilla text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
