...
// frame forever and the game appears frozen (MC's Il2Cpp trampoline handler only
...
internal static class SharedDesignBrowserCrashGuard
{
    ...
    internal static void NoteRefreshFailure(int year, PlayerData? nation, Exception ex)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                "UADMC shared-design browser guard: RefreshSharedDesign failed for year=" + year +
    ...
    internal static void NoteUpdateConstructorFailure(Exception ex)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                "UADMC shared-design browser guard: swallowed Ui.UpdateConstructor exception (" +
                ex.GetType().Name + ": " + ex.Message +
