...
{
    private const string LogPrefix = "UADMC smart refits";
    private const string AiLogPrefix = "UADMC smart refits ai";
    private const string AiDebugLogPrefix = "UADMC smart refits ai-debug";
    ...
    internal static void FlushPendingAiRefitTurnSummariesAfterNextTurn()
    {
        ...
        if (summaries.Count == 0)
        {
            ...
            if (LoggedEmptyAiRefitSummaries.Add(key))
                Melon<MintChipPlusMod>.Logger.Msg($"{AiLogPrefix} summary turn={LogToken(turn)} entries=0");
            return;
        }
        ...
        foreach (AiSmartRefitTurnSummary summary in summaries)
        {
            try
            {
                Melon<MintChipPlusMod>.Logger.Msg(summary.ToLogLine());
    ...
    private static void LogAi(string message)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"{AiLogPrefix}: {message}");
    }
...
internal static class CampaignSmartRefitAiRefitShipsPatch
{
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: CampaignController.AiRefitShips target not found; AI smart refit replacement disabled.");
        else
            Melon<MintChipPlusMod>.Logger.Msg("UADMC smart refits: attached CampaignController.AiRefitShips smart replacement.");
...
internal static class CampaignSmartRefitAiRefitShipsMoveNextPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: CampaignController.AiRefitShips MoveNext target not found; AI refit block disabled.");
        else
            Melon<MintChipPlusMod>.Logger.Msg("UADMC smart refits: attached CampaignController.AiRefitShips MoveNext fallback guard.");
...
internal static class CampaignSmartRefitStartPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: PlayerController.RefitShipsStart target not found; defensive non-player refit block disabled.");
...
internal static class CampaignSmartRefitCampaignDeleteDesignGuardPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: CampaignController.DeleteDesign target not found; live-referenced AI refit delete guard disabled.");
...
internal static class CampaignSmartRefitPlayerDeleteDesignGuardPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: PlayerController.DeleteDesign target not found; live-referenced AI refit delete guard disabled.");
...
internal static class CampaignSmartRefitSummaryNextTurnCompletionPatch
{
    ...
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits ai: NextTurn MoveNext target not found; summary flush will be skipped.");
...
internal static class CampaignSmartRefitCanBuildDesignAmountPatch
{
    ...
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: PlayerController.CanBuildShipsFromDesign amount target not found; AI refit design buildability gate disabled.");
...
internal static class CampaignSmartRefitCanBuildDesignPatch
{
    ...
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refits: PlayerController.CanBuildShipsFromDesign target not found; AI refit design buildability gate disabled.");
