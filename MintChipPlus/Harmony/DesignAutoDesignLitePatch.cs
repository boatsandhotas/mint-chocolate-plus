...
{
    private const string LogPrefix = "UADMC auto design lite";
    private const string ButtonName = "UADMC_AutoDesignLite";
        ...
        try
        {
            ...
            if (source == null)
            {
                if (!loggedMissingButton)
                {
                    ...
                    Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: vanilla RandomShip constructor button not found; Lite button disabled.");
                }
            ...
            {
                loggedAttached = true;
                Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: attached constructor button.");
            }
    ...
    private static void RunAutoDesignLite()
    {
        ...
        if (AddRandomPartsNewMethod == null)
        {
            if (!loggedMissingPartsMethod)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: Ship.AddRandomPartsNew target not found; Lite button disabled.");
            }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: skipped; constructor is not editable for the main player.");
            return;
        }
...
internal static class DesignAutoDesignLiteConstructorUiPatch
{
    ...
    private static bool Prepare()
    {
        ...
        if (!found)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC auto design lite: Ui.ConstructorUI target not found; button disabled.");
        return found;
    }
