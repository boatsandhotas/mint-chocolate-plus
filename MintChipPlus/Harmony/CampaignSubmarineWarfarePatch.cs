...
internal static class CampaignSubmarineConstructionPatch
{
    ...
    private static void LogFirstBuildBlock(SubmarineType? type, Player? player)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: blocking submarine construction because Submarine Warfare is Disabled. First blocked type: {typeName}; player: {playerName}.");
    }
}
...
internal static class CampaignAiSubmarineConstructionPatch
{
    ...
    private static bool BuildNewSubmarinesPrefix(Player player)
    {
        ...
        if (!loggedFirstAiBuildBlock)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: skipping monthly submarine construction while disabled. First skipped player: {SubmarineWarfareUtility.PlayerLabel(player)}.");
        }
...
internal static class CampaignSubmarineBattleGenerationPatch
{
    ...
    private static void LogFirstPublicGenerationBlock(Player? a, Player? b, BattleTypeEx? battleType)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC submarine warfare: blocking submarine battle generation while disabled. First blocked battle: {SubmarineWarfareUtility.BattleTypeLabel(battleType)}; {SubmarineWarfareUtility.PlayerLabel(a)} vs {SubmarineWarfareUtility.PlayerLabel(b)}.");
    }
}
...
internal static class CampaignSubmarineBattleAddPatch
{
    ...
    private static bool AddBattlePrefix(CampaignBattleWithSubmarine battle)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC submarine warfare: failed to release a suppressed submarine battle before registration. {ex.GetType().Name}: {ex.Message}");
        }

        if (!loggedFirstPrivateGenerationBlock)
        {
            loggedFirstPrivateGenerationBlock = true;
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: suppressing submarine battle registration while disabled. First blocked battle id: {SubmarineWarfareUtility.BattleLabel(battle)}.");
        }
...
internal static class CampaignSubmarineBattleResolvePatch
{
    ...
    private static bool CalculateDamagePrefix(CampaignBattleWithSubmarine battle)
    {
        ...
        if (!loggedFirstDamageSuppress)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: suppressing submarine battle damage while disabled. First battle id: {SubmarineWarfareUtility.BattleLabel(battle)}.");
        }
    ...
    private static void DismissBattle(BattleManager manager, CampaignBattleWithSubmarine battle, string context)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC submarine warfare: failed to dismiss disabled submarine battle during {context}. {ex.GetType().Name}: {ex.Message}");
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: dismissed an existing submarine battle during {context} because Submarine Warfare is Disabled. Battle id: {SubmarineWarfareUtility.BattleLabel(battle)}.");
    }
    ...
    private static bool DamageSubmarinePrefix(Submarine sub)
    {
        ...
        if (!loggedFirstSubmarineDamageSuppress)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: suppressing direct submarine damage while disabled. First submarine: {SubmarineWarfareUtility.SubmarineLabel(sub)}.");
        }
...
internal static class CampaignSubmarineBattleCompletePatch
{
    ...
    private static bool CompleteBattlePrefix(BattleManager __instance, CampaignBattleWithSubmarine battle)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC submarine warfare: failed to dismiss disabled submarine battle during completion. {ex.GetType().Name}: {ex.Message}");
        }

        if (!loggedFirstCompleteDismiss)
        {
            loggedFirstCompleteDismiss = true;
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC submarine warfare: dismissed a submarine battle during completion because Submarine Warfare is Disabled. Battle id: {SubmarineWarfareUtility.BattleLabel(battle)}.");
        }
...
internal static class CampaignSubmarineBuildReasonPatch
{
    ...
    private static bool CanNotBuildSubmarineReasonToUiPrefix(string reason, ref string __result)
    {
        ...
        __result = "Submarine warfare is disabled by UAD:MC.";
        return false;
    }
}
