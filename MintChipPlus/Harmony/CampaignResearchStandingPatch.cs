...
internal static class CampaignResearchStandingPatch
{
    ...
    private const string BadgeName = "UADMC_ResearchStandingBadge";
    ...
    internal static void ApplyToWindow(CampaignResearchWindow? window, string context)
    {
        try
        {
            ...
            if (decorated > 0 && logSummary != lastLoggedSummary)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC research standing: decorated {decorated} categories after {context}. {string.Join("; ", samples)}");
            }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC research standing refresh failed; leaving vanilla research text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
