        ...
        try
        {
            ...
            if (summary != lastLoggedSummary)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC campaign technology status: displayed next discovery {summary} in {stopwatch.ElapsedMilliseconds} ms. Candidates: {estimate.DebugDetails}");
            }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign technology status patch failed; leaving vanilla text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
