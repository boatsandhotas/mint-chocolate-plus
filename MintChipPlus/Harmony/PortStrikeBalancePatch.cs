...
// target pool, so a lone small raider can erase far too many transports. MC
// keeps the mission but caps resulting TR losses by the attacker's committed
// tonnage, then scales the transport MC to match the adjusted loss count.
...
internal static class PortStrikeBalancePatch
{
    ...
    private static void PostfixCalcVp(CampaignBattle battle)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC port strike balance failed; keeping vanilla result. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void BalanceDefenderTransportLosses(CampaignBattle battle)
    {
        ...
        if (balancedLosses >= vanillaLosses)
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC port strike balance: no clamp needed. attackerTons={attackerTonnage:0}, vanillaTR={vanillaLosses}.");
            return;
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC port strike balance: attackerTons={attackerTonnage:0}, vanillaTR={vanillaLosses}, balancedTR={balancedLosses}, transportVP={oldTransportVp:0.##}->{newTransportVp:0.##}.");
    }
    ...
    private static float TotalTonnage(Il2CppSystem.Collections.Generic.List<Ship>? ships)
    {
        ...
        foreach (Ship ship in ships)
        {
            ...
            {
                // Some generated/temporary ships can be incomplete during MC
