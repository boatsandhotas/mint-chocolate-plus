...
internal static class CampaignAiArmsRacePatch
{
    ...
    private static bool TryRemoveGeneratedDesign(Player player, Ship design)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC AI Arms Race: CampaignController.DeleteDesign failed for {DesignLabel(design)}. {ex.GetType().Name}: {ex.Message}");
        }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC AI Arms Race: design erase failed for {DesignLabel(design)}. {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static void LogBuildBlocked(Ship design, AiDesignCompetitiveness.CompetitivenessInfo info)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC AI Arms Race blocked build: turn={AiDesignCompetitiveness.CurrentTurnLabel()} nation={AiDesignCompetitiveness.PlayerLabel(design.player)} type={info.Type} design={AiDesignCompetitiveness.ShipLabel(design)} base={AiDesignCompetitiveness.FormatCompactPower(info.BasePower)} adjusted={AiDesignCompetitiveness.FormatCompactPower(info.Estimate)} benchmark={AiDesignCompetitiveness.FormatCompactPower(info.Benchmark)} ratio={AiDesignCompetitiveness.FormatRatio(info.Ratio)} threshold={AiDesignCompetitiveness.FormatRatio(AiDesignCompetitiveness.MinimumCompetitiveRatio)} reason=uncompetitive.");
    }
...
internal static class CampaignAiArmsRaceGenerateRandomDesignsPatch
{
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC AI Arms Race: GenerateRandomDesigns target not found; generated-design rejection disabled.");
...
internal static class CampaignAiArmsRaceBuildNewShipsPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC AI Arms Race: BuildNewShips target not found; generated-design rejection disabled.");
...
internal static class CampaignAiArmsRaceCanBuildShipsFromDesignPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC AI Arms Race: CanBuildShipsFromDesign target not found; build gate disabled.");
