...
internal static class SharedDesignHistoricalYearPatch
{
        ...
        ref int result)
    {
        ...
        catch (Exception ex)
        {
            WarnOnce(
                ...
                $"UADMC shared-design historical years: failed to read deterministic tech year for {SafeTechKey(tech)}. {ex.GetType().Name}: {ex.Message}");
            return true;
        }
        finally
        {
    ...
    private static void LogActiveOnce()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            "UADMC shared-design historical years: using deterministic tech years in Shared Designs screen.");
    }
