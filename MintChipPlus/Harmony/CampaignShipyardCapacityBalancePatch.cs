...
// shipyard job. MC instead lets the monthly advance run only the work that fits
...
internal static class CampaignShipyardCapacityBalancePatch
{
    ...
    private static AdvancePlan? BeginAdvance(Player player, bool prewarm)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC shipyard capacity balance failed before monthly ship advance; keeping vanilla behavior. {ex.GetType().Name}: {ex.Message}");
            return null;
        }
    }
    ...
    private static AdvancePlan? CreateAdvancePlan(Player player)
    {
        ...
        if (!player.isAi)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC shipyard capacity balance: {player.Name(false)}, capacity={capacity:0}t, queued={work.Count} ships/{activeTonnage:0}t, running={runningSummary.LogText}, delayed={delayedSummary.LogText}.");
        }
    ...
    private static WorkItem? TryCreateWorkItem(Ship ship)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC shipyard capacity balance skipped one work item. {ex.GetType().Name}: {ex.Message}");
        }
