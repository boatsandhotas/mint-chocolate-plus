...
internal static class PlayerDeletedDesignClonePatch
{
    ...
    private static void CloneShipRawPostfix(Ship from, bool willBeDesign, Ship __result)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC refit design clone: failed to clear inherited deleted state. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogClearedDeletedFlagOnce(Ship source, Ship clone)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC refit design clone: cleared inherited deleted state for new player design '{Safe(() => clone.name, "?")}' cloned from '{Safe(() => source.name, "?")}'.");
    }
