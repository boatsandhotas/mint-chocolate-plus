...
internal static class CampaignGlobeVisualPatch
{
    ...
    private static void BuildGlobe(CampaignMap map)
    {
        try
        {
            ...
            GlobeRoot = new GameObject("UADMC_Globe");
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC_GLOBE political globe built: radius={radius:0} center={center} k={calK:0.####} latC=[{cll00.x:0.#},{cll11.x:0.#}] lonC=[{cll00.z:0.#},{cll11.z:0.#}] land-countries={landCountries}.");
            try
            {
                var s = map.Settings;
                if (s != null)
                    Melon<MintChipPlusMod>.Logger.Msg(
                        $"UADMC_GLOBE map settings: lonL={s.MapLongitudeLeft:0.##} lonR={s.MapLongitudeRight:0.##} latBottom={s.MapLongitudeBottom:0.##} W={s.MapWidth:0} H={s.MapHeight:0} offset={s.MapOffset} landZ=[{calMinZ:0},{calMaxZ:0}].");
            }
            catch (Exception sx) { Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE settings read failed: {sx.GetType().Name}"); }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE build failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void BuildOcean()
    {
        ...
        if (haveTex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE ocean texture='{mapTex!.name}' {mapTex.width}x{mapTex.height} mapBounds=({mb.min.x:0},{mb.min.z:0})..({mb.max.x:0},{mb.max.z:0})");
        }

        GameObject ocean = new("UADMC_GlobeOcean");
        ocean.layer = GlobeRoot!.layer;
        ocean.transform.SetParent(GlobeRoot.transform, false);
        ocean.AddComponent<MeshFilter>().mesh = mesh;
        ocean.AddComponent<MeshRenderer>().sharedMaterial = CreateUnlitColorMaterial(OceanColor, "UADMC_GlobeOcean_Mat");
        ...
        if (haveTex && mapTex != null)
        {
            ...
            if (tm != null)
            {
                var overlay = new GameObject("UADMC_GlobeTerrain");
                ...
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE terrain overlay shader='{tm.shader.name}'");
            }
        }
    }
    ...
    private static Material? CreateTerrainOverlayMaterial(Texture tex, float alpha)
    {
        try
        {
            ...
            var m = new Material(s) { name = "UADMC_GlobeTerrain_Mat" };
    ...
    private static int BuildLand(CampaignMap map)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE land build: {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static void BuildBorderMesh()
    {
        try
        {
            if (GlobeRoot == null || borderVerts.Count < 2)
                return;
            Transform existing = GlobeRoot.transform.Find("UADMC_GlobeBorders");
            if (existing != null)
            ...
            GameObject go = new("UADMC_GlobeBorders");
            go.layer = GlobeRoot.layer;
            go.transform.SetParent(GlobeRoot.transform, false);
            go.AddComponent<MeshFilter>().mesh = mesh;
            Material bmat = CreateUnlitColorMaterial(BorderColor, "UADMC_GlobeBorders_Mat");
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE borders: {borderIdx.Count / 2} edges.");
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE borders: {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static bool ReprojectCountryMesh(MeshRenderer src, string countryName, int idx, bool countryHasColored)
    {
        try
        {
            ...
            GameObject go = new($"UADMC_GlobeLand_{countryName.Replace(' ', '_')}_{idx}");
            go.layer = GlobeRoot!.layer;
            go.transform.SetParent(GlobeRoot.transform, false);
            go.AddComponent<MeshFilter>().mesh = mesh;
    ...
    {
        if (root == null) { Melon<MintChipPlusMod>.Logger.Msg($"marker-root {tag}=null"); return; }
        int n = root.childCount;
        Melon<MintChipPlusMod>.Logger.Msg($"marker-root {tag}='{root.gameObject.name}' children={n}");
        for (int i = 0; i < n && i < 4; i++)
        {
            try
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg($"  {tag}[{i}] '{ch.gameObject.name}' active={ch.gameObject.activeInHierarchy} pos={ch.position} euler={ch.eulerAngles} childVerts={vc}");
            }
            catch { }
        }
    }
        ...
        try
        {
            ...
            if (all != null)
                for (int i = 0; i < all.Length && nn < 24; i++)
                {
                    ...
                    nn++;
                    Melon<MintChipPlusMod>.Logger.Msg($"marker-cand '{nm}' active={act} verts={vc} parent={(all[i].parent != null ? all[i].parent.gameObject.name : "-")}");
                }
    ...
    private static void DumpSubtree(Transform t, int depth, int maxDepth)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"  pb-sub d{depth} '{t.gameObject.name}' act={t.gameObject.activeInHierarchy} verts={vc} lr={lr} mr={mr} spr={spr} mat='{mat}' pos={t.position}");
            if (depth < maxDepth)
    ...
    internal static void DumpGlobeState()
    {
        try
        {
            var L = Melon<MintChipPlusMod>.Logger;
            L.Msg("=== UADMC_GLOBE STATE ===");
            ...
            if (GlobeRoot != null)
            {
                ...
                for (int i = 0; i < t.childCount; i++)
                {
                    ...
                    string nm = ch.name;
                    if (nm == "UADMC_GlobeBorders") borderMesh = ch;
                    if (!nm.StartsWith("UADMC_GlobeLand_")) continue;
                    landCount++;
                    if (landCount <= 8)
                    {
            ...
            try
            {
                ...
                for (int i = 0; i < rends.Length && shown < 40; i++)
                {
                    ...
                    if (nm.StartsWith("UADMC_")) continue;
        ...
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE state dump: {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    {
        if (root == null) { Melon<MintChipPlusMod>.Logger.Msg($"labels-{tag}=null"); return; }
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"labels-{tag} count={n} active={active} spread=({mx.x - mn.x:0},{mx.y - mn.y:0},{mx.z - mn.z:0})");
    }
    ...
    private static void DumpBorderCandidates(CampaignMap map)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < all.Length && n < 50; i++)
            {
                ...
                n++;
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE border-candidate: '{nm}' active={active} rend={hasR}/en={rEn} graphic={hasG} parent={(tr.parent != null ? tr.parent.gameObject.name : "-")}");
            }
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE border-candidates dumped ({n}).");
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE border-dump: {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static void HideFlat(CampaignMap map)
    {
        try
        {
            ...
            // Renderer) so they survive; our own globe objects (UADMC_*) and GlobeRoot are skipped.
            ...
            try
            {
                ...
                for (int i = 0; i < arr.Length; i++)
                {
                    ...
                    if (nm.StartsWith("UADMC_")) continue;            // never disable our globe
            ...
            catch (Exception rex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE renderer-hide: {rex.GetType().Name}: {rex.Message}"); }
            ...
            try { Renderer mr = map.MapRenderer; if (mr != null && mr.enabled) { hiddenRenderers.Add(mr); mr.enabled = false; Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE disabled MapRenderer '{mr.gameObject.name}' (root={mr.transform.root.gameObject.name}, mapRoot={map.transform.root.gameObject.name})"); } } catch { }
            try
            {
                ...
                if (bm2 != null)
                {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < rends.Length; i++)
                        {
                            ...
                            if (!match) continue;
                            try { if (r.gameObject.name.StartsWith("UADMC_")) continue; } catch { }
                            try { if (r.enabled) { hiddenRenderers.Add(r); r.enabled = false; } } catch { }
                            hit++;
                            try { Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE hid BORDER renderer '{r.gameObject.name}' mat={mn} parent={(r.transform.parent != null ? r.transform.parent.gameObject.name : "-")}"); } catch { }
                        }
                    }
                    catch { }
                    Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE borders: DrawAllBorders false (was {savedDrawAllBorders}); material={(bordersMat != null ? bordersMat.name : "null")} shader={shd} by-material-hits={hit}");
                }
            }
            catch { }
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE hid {hiddenRenderers.Count} flat-map renderer(s).");
            ...
            try
            {
                ...
                for (int i = 0; i < cams.Length; i++)
                {
                    ...
                    Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE disabled camera '{cc.name}' rt={rt} parent={par} comps=[{comps}]");
        ...
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE hide-flat: {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    internal static void DriveOrbitCamera(Cam cam)
    {
        try
        {
            ...
            {
                driveLogCount++;
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE drive frame {driveLogCount}: beforeWrite={cam.transform.position} ortho={cam.cameraComp.orthographic} far={cam.cameraComp.farClipPlane:0}");
            }
            ...
            if (driveLogCount == 1)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC_GLOBE camera drive engaged: center={c} camPos={camPos} dist={dist:0} ortho={cam.cameraComp.orthographic} camComp==rig={sameGo} camParent={parent} readback={camT.position}");
            }
        }
        catch { }
    }
    ...
    internal static void MaybeRebuildLand()
    {
        ...
        try
        {
            ...
            for (int i = root.childCount - 1; i >= 0; i--)
            {
                ...
                if (nm.StartsWith("UADMC_GlobeLand_"))
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE land re-cloned to catch late coloring: {countries} countries.");
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE land rebuild: {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static void BuildZoneRings()
    {
        ...
        zoneRingObj = new GameObject("UADMC_GlobeZoneRings");
        zoneRingObj.layer = GlobeRoot.layer;
        zoneRingObj.transform.SetParent(GlobeRoot.transform, false);
        if (!zoneColorLogged)
        {
            zoneColorLogged = true;
            foreach (KeyValuePair<Color, List<Vector3>> kv in byColor) Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE zone-ring color=({kv.Key.r:0.00},{kv.Key.g:0.00},{kv.Key.b:0.00}) segs={kv.Value.Count / 2}");
            ...
            try { Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE zone roots: denial={CircleInfo(denialZoneRoot)} minesweep={CircleInfo(mineSweepRoot)} events={CircleInfo(eventsRoot)}"); } catch { }
        ...
        foreach (KeyValuePair<Color, FillBucket> kv in byColorFill)
        {
            ...
            GameObject? fgo = BuildFillObject(fb.V, fb.T, new Color(kv.Key.r, kv.Key.g, kv.Key.b, 0.16f), "UADMC_GlobeZoneFill");
        ...
        foreach (KeyValuePair<Color, List<Vector3>> kv in byColor)
        {
            ...
            go.AddComponent<MeshFilter>().mesh = mesh;
            Material rmat = CreateUnlitColorMaterial(kv.Key, "UADMC_GlobeZoneRing_Mat");
    ...
    {
        if (root == null) { Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE {tag}-children: root null"); return; }
        try
        {
            ...
            for (int i = 0; i < root.childCount; i++)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE {tag}-child '{ch.gameObject.name}' rend={(r != null ? r.GetType().Name : "none")} rad={rad:0.000} col=({mc.r:0.00},{mc.g:0.00},{mc.b:0.00},{mc.a:0.00})");
            }
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE {tag}-children: active={active}/{root.childCount}");
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE {tag}-children err {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static void ReprojectBattleFlags()
    {
        ...
        if (Time.frameCount >= battleDiagFrame) { battleDiagFrame = Time.frameCount + 180; Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE battle-flags active={fa} moved={fm} firstPosMag={firstMag:0.0} (globeR~{radius * LandLiftFactor:0.0}) firstRendEn={firstRend}"); }
    }
            ...
            if (hoverId.Length > 0)
            {
                    ...
                    && LandInvasionSupport.PlayerSideInBattle(pb) != null)
                {
                    ...
                    try
                    {
                        ...
                        Melon<MintChipPlusMod>.Logger.Msg(
                            $"UADMC_REINFORCE click-toggle -> {(added ? "reinforcing" : "stopped")} " +
                    ...
                    catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_REINFORCE click err {ex.GetType().Name}: {ex.Message}"); }
                }
                ...
                if (hoverId != lastHoverLogged)
                {
                    ...
                    Melon<MintChipPlusMod>.Logger.Msg(
                        $"UADMC_GLOBE battle-hover suffix='{hoverId}' pbFound={(pb != null)} strat={usedStrategy} " +
    ...
    private static void PopulateBattlePopupManually(CampaignProvinceBattlePopupUI popup, ProvinceBattle pb)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE manual popup populate failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void BuildBattleArrows()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            try
            {
                ...
                if (doProbe && probed < 6)
                {
                    ...
                    Melon<MintChipPlusMod>.Logger.Msg(
                        $"UADMC_GLOBE battle-arrow '{ch.gameObject.name}' pc={pc} ws={ws} " +
        ...
        battleArrowObj = null;
        if (verts.Count < 3) { if (doProbe && probed > 0) Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE battle-arrows MESH: verts={verts.Count} tris={tris.Count} -> too few, NO mesh"); return; }
        var mesh = new Mesh { indexFormat = UnityEngine.Rendering.IndexFormat.UInt32 };
        mesh.vertices = verts.ToArray();
        mesh.triangles = tris.ToArray();
        mesh.RecalculateNormals();
        mesh.bounds = new Bounds(Vector3.zero, Vector3.one * (radius * 4f));
        battleArrowObj = new GameObject("UADMC_GlobeBattleArrows");
        ...
        Material amat = CreateUnlitColorMaterial(BattleArrowColor, "UADMC_GlobeBattleArrow_Mat");
        try { amat.renderQueue = 2600; } catch { } // over the fills/terrain
        battleArrowObj.AddComponent<MeshRenderer>().sharedMaterial = amat;
        if (doProbe && probed > 0)
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE battle-arrows MESH built: verts={verts.Count} tris={tris.Count} obj=True rq={amat.renderQueue} radius={radius:0.0}");
    }
    ...
    private static void BuildReinforceCircles(Vector3 c)
    {
        try
        {
            ...
            reinforceFillObj = BuildFillObject(fillV, fillT, ReinforceFillColor, "UADMC_GlobeReinforceFill");
            reinforceCircleObj = BuildLineLoopObject(borderSegs, ReinforceCircleColor, "UADMC_GlobeReinforceBorder");
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_GLOBE reinforce circles failed. {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static void ReprojectCircles()
    {
        if (!circleProbed)
        {
            ...
            try
            {
                Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE circle roots: mineSweep={CircleInfo(mineSweepRoot)} denial={CircleInfo(denialZoneRoot)} events={CircleInfo(eventsRoot)}");
            }
            catch { }
        }
    ...
    internal static void ProjectMarkerToGlobe(Vector3 worldPos, ref Vector3 uiPos, ref float scale)
    {
        try
        {
            ...
            if (markerScaleLog < 12) { markerScaleLog++; Melon<MintChipPlusMod>.Logger.Msg($"UADMC_GLOBE marker native scale={scale:0.###} dist={dist:0}"); }
