...
internal static class CampaignFleetIncomingPurchasesPatch
{
    ...
    private static void PostfixRefresh(CampaignFleetWindow __instance, bool isDesign)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC_ALLYBUY fleet-row inject failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static GameObject? BuildRow(CampaignFleetWindow window, FleetWindow_ShipElementUI source, Transform parent, AllyPurchaseState.Order o, Ship hull)
    {
        ...
        clone.name = "UADMC_IncomingPurchase";
    ...
    internal static void OpenPortPickerForOrders(CampaignFleetWindow window, List<AllyPurchaseState.Order> orders, System.Action? refresh = null)
    {
        try
        {
            ...
            if (sel == null) { Melon<MintChipPlusMod>.Logger.Warning("UADMC_ALLYBUY: no SelectionWindow on fleet window"); return; }
            ...
            sel.Show(title, options,
                new System.Action<int>(i =>
                {
                    ...
                    catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_ALLYBUY port-pick: {ex.GetType().Name}: {ex.Message}"); }
                }),
                null, dict);
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_ALLYBUY OpenPortPickerForOrders: {ex.GetType().Name}: {ex.Message}"); }
    }
    ...
    private static void ConfirmCancel(AllyPurchaseState.Order o, System.Action refresh)
    {
        try
        {
            ...
            MessageBoxUI.Show($"Cancel order — {o.DesignName}", text, null, false, "Cancel order", "Keep",
                new System.Action(() => { try { AlliedShipPurchase.CancelOrder(o); refresh(); } catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_ALLYBUY cancel: {ex.GetType().Name}: {ex.Message}"); } }),
                null);
        }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC_ALLYBUY ConfirmCancel: {ex.GetType().Name}: {ex.Message}"); }
    }
