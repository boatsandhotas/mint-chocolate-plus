...
internal static class CampaignHistoricalResearchPatch
{
    ...
    private static void SynchronizeHistoricalTechs(string context, CampaignController? campaign)
    {
        ...
        if (pass > MaxSyncPasses)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC historical research: stopped after {MaxSyncPasses} passes at year {currentYear}; additional due techs may remain.");
        }

        HistoricalResearchAudit audit = AuditHistoricalResearch(players, dueData);

        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC historical research: {currentYear} synced {syncedPlayers} nations, completed {completedTotal} technologies (existing={completedExistingTotal}, addedMissingDue={addedMissingDueTotal}), budgets zeroed, changedBudgets={budgetChanges}, passes={Math.Min(pass, MaxSyncPasses)}, context={context}.");

        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC historical research audit: year={currentYear} players={players.Count} dueData={audit.DueDataCount} completedExisting={completedExistingTotal} addedMissingDue={addedMissingDueTotal} missingDueAfter={audit.MissingDueAfter} budgetsZeroed={syncedPlayers} changedBudgets={budgetChanges} passes={Math.Min(pass, MaxSyncPasses)} context={context}.");
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC historical research samples: {samples}.");
    }
    ...
    private static void ClampHistoricalResearchSpending(string context, CampaignController? campaign)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC historical research: clamped research spending for {syncedPlayers} nations, changedBudgets={changedBudgets}, context={context}.");
    }
        ...
        if (completedExisting > 0 || addedMissingDue > 0)
        {
            ...
            catch (Exception ex)
            {
                WarnOnce(
                    ...
                    $"UADMC historical research: OnTechnologiesChanged failed for {PlayerLabel(player)}. {ex.GetType().Name}: {ex.Message}");
            }
        }
    ...
    private static List<TechnologyData> DueHistoricalTechData(int currentYear)
    {
        ...
        catch (Exception ex)
        {
            WarnOnce(
                ...
                $"UADMC historical research: failed to enumerate global technologies for historical sync. {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static void WarnForMissingDueTechnologies(int currentYear, HistoricalResearchAudit audit)
    {
        ...
        foreach (PlayerMissingDueAudit player in audit.MissingPlayers.Take(8))
        {
            ...
            WarnOnce(
                ...
                $"UADMC historical research audit WARNING: year={currentYear} {player.PlayerLabel} missingDueAfter={player.MissingCount} samples={samples}.");
        }
    }
    ...
    private static void LogTrackedTechProbe(int currentYear, IReadOnlyList<Player> players, IReadOnlyList<TechnologyData> dueData)
    {
        ...
        if (missingByPlayer.Count == 0)
        {
            ...
            {
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC historical research probe: year={currentYear} tracked due techs verified for {players.Count} nations.");
            }
        ...
        WarnOnce(
            ...
            $"UADMC historical research probe WARNING: year={currentYear} {string.Join("; ", missingByPlayer.Take(8))}.");
    }
    ...
    private static void ReportPlayerDiscoveries(string context, List<Technology> completedTechnologies)
    {
        ...
        if (mainPlayer == null || ui == null)
        {
            WarnOnce(
                ...
                $"UADMC historical research: could not report {completedTechnologies.Count} player discoveries because campaign UI is unavailable, context={context}.");
            return;
        }
        ...
        foreach (Technology tech in completedTechnologies)
        {
            ...
            catch (Exception ex)
            {
                ...
                WarnOnce(
                    ...
                    $"UADMC historical research: ReportDiscovery failed for {TechnologyName(tech)}. {ex.GetType().Name}: {ex.Message}");
            }
        }

        string failedText = failed > 0 ? $", failed={failed}" : string.Empty;
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC historical research: reported {reported}/{completedTechnologies.Count} player discoveries, context={context}{failedText}.");
    }
    ...
    private static int HistoricalTechYear(TechnologyData data)
    {
        ...
        catch (Exception ex)
        {
            WarnOnce(
                ...
                $"UADMC historical research: failed to read historical year for {TechLabel(data)}. {ex.GetType().Name}: {ex.Message}");
            return int.MaxValue;
        }
    }
    ...
    private static void RefreshVanillaResearchState(CampaignController campaign)
    {
        ...
        catch (Exception ex)
        {
            WarnOnce(
                ...
                $"UADMC historical research: TechStartNewResearchs invoke failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void ZeroResearchSpending(Player player, ref int budgetChanges)
    {
        ...
        catch (Exception ex)
        {
            WarnOnce(
                ...
                $"UADMC historical research: failed to zero tech budget for {PlayerLabel(player)}. {ex.GetType().Name}: {ex.Message}");
        }
        ...
        catch (Exception ex)
        {
            WarnOnce(
                ...
                $"UADMC historical research: failed to clear tech priorities for {PlayerLabel(player)}. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void WarnOnce(string key, string message)
    {
        if (LoggedWarnings.Add(key))
            Melon<MintChipPlusMod>.Logger.Warning(message);
    }
