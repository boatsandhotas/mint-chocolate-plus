...
internal static class CampaignArmyLogisticsBalancePatch
{
    ...
    private static void LogSample(Player player, LogisticsBreakdown breakdown)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC army logistics: mode=Balanced player={LogToken(AiDesignCompetitiveness.PlayerLabel(player))} class={breakdown.Class} " +
    ...
    private static void LogFailureOnce(Exception ex)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning(
            $"UADMC army logistics: balanced calculation failed; falling back to vanilla for this call. {ex.GetType().Name}: {ex.Message}");
    }
