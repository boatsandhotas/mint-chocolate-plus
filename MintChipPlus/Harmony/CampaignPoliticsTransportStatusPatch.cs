...
internal static class CampaignPoliticsTransportStatusPatch
{
    ...
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC politics transport status unavailable: CampaignPolitics_ElementUI.GetNavalInfo not found.");
    ...
    private static void LogActiveOnce()
    {
        ...
        loggedActive = true;
        Melon<MintChipPlusMod>.Logger.Msg("UADMC politics transport status active: appended transport capacity to naval info rows.");
    }
    ...
    private static void LogFailureOnce(Exception ex)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning(
            $"UADMC politics transport status failed; leaving vanilla naval info intact. {ex.GetType().Name}: {ex.Message}");
    }
}
