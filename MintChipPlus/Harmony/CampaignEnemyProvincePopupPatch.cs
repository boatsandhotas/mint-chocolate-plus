...
// does nothing. MC brackets OnPointerClick with a Prefix + Postfix that
...
internal static class CampaignEnemyProvincePopupPatch
{
    ...
    private static void Prefix(MapTextMeshLabel __instance)
    {
        ...
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC province-label click #{clicksObserved}: label='{labelId}' province='{provId}'.");
        }
        catch { }
    }
    ...
    private static void Postfix(MapTextMeshLabel __instance, PointerEventData eventData)
    {
        try
        {
            ...
            {
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC enemy-province-popup: click on label '{__instance.Id}' but province is null; skipping.");
                return;
            }
            ...
            if (popup == null)
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    "UADMC enemy-province-popup: G.ui.ProvincePopupElement is null.");
                return;
            }
            ...
            if (lastLoggedProvinceId != province.Id)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC enemy-province-popup: forcing popup for province={province.Id} controller={controller}.");
            }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC enemy-province-popup: postfix failed. {ex.GetType().Name}: {ex.Message}");
        }
        finally
        {
