...
{
    private const string AiControlButtonName = "UADMC_OrderAiControl";
    ...
    internal static void Reset(string context)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC battle AI control: cleared {count} division toggle(s) during {context}.");
    }
    ...
    internal static void RefreshAiControlOrder(Ui ui)
    {
        ...
        if (canToggle && !loggedButtonReady)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC battle AI control: AI button ready for {divisions.Count} selected division(s); root={orderContext.SourceName}/{orderContext.Root.name}, rootActive={orderContext.Root.activeInHierarchy}, parent={parent?.name ?? "<none>"}, parentActive={parent?.gameObject.activeInHierarchy ?? false}, template={orderContext.Template.gameObject.name}, anchor={orderContext.Anchor?.gameObject.name ?? "<none>"}, buttonActive={button.gameObject.activeSelf}.");
        }
        ...
        if (canToggle && !loggedInactiveButtonParent && !(button.transform.parent?.gameObject.activeInHierarchy ?? false))
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle AI control: AI button parent is inactive after placement; roots={DescribeOrderRoots(ui)}.");
        }
    }
    ...
    private static void ToggleSelectedDivisions(Ui ui, Button? button, string context = "button")
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC battle AI control: {(enable ? "enabled" : "disabled")} AI control for {divisions.Count} selected division(s), {shipCount} ship(s) via {context}.");
    }
    ...
    private static void ClearSelectedAiControl(Ui ui, string context)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC battle AI control: disabled AI control for {divisions.Count} selected division(s), {shipCount} ship(s) via {context}.");
    }
        ...
        IReadOnlyList<(string SourceName, GameObject Root)> roots)
    {
        ...
        if (!loggedButtonAdded)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg("UADMC battle AI control: added AI control button to battle division orders.");
        }
    ...
    internal static void PositionAiControlButton(Button button, OrderButtonContext orderContext)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle AI control: could not position AI button; using cloned layout. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void TryRegisterMiniHotkey(Button button)
    {
        ...
        catch (Exception ex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle AI control: could not register AI hotkey {AiControlHotkey}. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogMissingUiFields()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning("UADMC battle AI control: could not read vanilla battle order UI fields; AI control button unavailable.");
    }
