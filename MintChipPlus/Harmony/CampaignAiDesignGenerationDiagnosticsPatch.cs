...
internal static class CampaignAiDesignGenerationDiagnostics
{
    ...
    private static void WarnOnce(string key, string message)
    {
        if (LoggedWarnings.Add(key))
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} {message}");
    }
...
internal static class CampaignAiDesignGenerationGenerateRandomDesignsPatch
{
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("[AI DesignGen] GenerateRandomDesigns target not found; design-generation diagnostics disabled.");
...
internal static class CampaignAiDesignGenerationBuildNewShipsPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("[AI DesignGen] BuildNewShips target not found; design-generation result diagnostics disabled.");
...
internal static class CampaignAiDesignGenerationGenerateRandomDesignsMoveNextPatch
{
    private static bool Prepare()
    {
        bool available = TargetMethod() != null;
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning("[AI DesignGen] GenerateRandomDesigns MoveNext target not found; gate diagnostics disabled.");
...
internal static class CampaignAiDesignGenerationRandomShipMoveNextPatch
{
    ...
    private static void Prepare()
    {
        if (TargetMethod() == null)
            Melon<MintChipPlusMod>.Logger.Warning("[AI DesignGen] Ship.GenerateRandomShip MoveNext target not found; random-attempt diagnostics disabled.");
    }
