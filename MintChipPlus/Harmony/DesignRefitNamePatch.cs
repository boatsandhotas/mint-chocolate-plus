...
// and even with MC's class-year disambiguation, two refits designed in the
...
internal static class DesignRefitNamePatch
{
        ...
        ref string __result)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC design refit names failed; using vanilla name. {ex.GetType().Name}: {ex.Message}");
            return true;
        }
    }
    ...
    // month is MC's own addition and stays internal-only here.
    ...
    private static void LogRuleOnce()
    {
        ...
        loggedRule = true;
        Melon<MintChipPlusMod>.Logger.Msg("UADMC design refit names: using class month+year naming (e.g. \"Foo (Jul. 1904)\") for player and AI refits.");
    }
    ...
    private static void LogConflictOnce(string generatedName)
    {
        ...
        loggedConflict = true;
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC design refit names: resolved same-year conflict as {generatedName}.");
    }
}
