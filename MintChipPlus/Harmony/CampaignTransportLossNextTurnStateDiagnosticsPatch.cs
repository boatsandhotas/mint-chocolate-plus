...
{
    private const string LogPrefix = "UADMC TR state diag";
    ...
    private static bool Prepare()
    {
        ...
        if (!available)
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix} unavailable: NextTurn state-machine target or transport-loss accessor missing.");
    ...
    private static void LogFailureOnce(string action, Exception ex)
    {
        ...
        lastFailure = failure;
        Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: {action} failed; diagnostic remains read-only. {ex.GetType().Name}: {ex.Message}");
    }
