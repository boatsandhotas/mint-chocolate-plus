...
using MintChipPlus.UserInterface;
using UnityEngine;
using UnityEngine.UI;

namespace MintChipPlus.Harmony;

// Player-only test surface for MC's conservative Smart Refit service. This is
...
{
    private const string LogPrefix = "UADMC smart refit";
    private const string ButtonName = "UADMC_SmartRefit";
    private const string AutoLiteButtonName = "UADMC_AutoDesignLite";
    private const string GenerateArmorButtonName = "UADMC_GenerateArmor";
        ...
        try
        {
            ...
            if (source == null)
            {
                ...
                if (!loggedMissingButton)
                {
                    ...
                    Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: constructor button template not found; Smart Refit button disabled.");
                }
            ...
            {
                loggedAttached = true;
                Melon<MintChipPlusMod>.Logger.Msg($"{LogPrefix}: attached player Smart Refit constructor button.");
            }
        }
        catch (Exception ex)
        {
    ...
    private static void RunSmartRefit()
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: skipped; constructor ship was unavailable.");
            return;
        }

        if (!ModSettings.SmartRefitsEnabled)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: skipped; Smart Refits option is Vanilla.");
            return;
        }

        if (!IsConstructor() || !Safe(() => ui.allowEdit, false) || !IsHumanMainDesignerShip(ship) || !IsRefitContext(ui, ship))
        {
            Melon<MintChipPlusMod>.Logger.Warning($"{LogPrefix}: skipped; active constructor is not an editable player refit.");
            return;
        }
...
internal static class DesignSmartRefitConstructorUiPatch
{
    ...
    private static bool Prepare()
    {
        ...
        if (!found)
            Melon<MintChipPlusMod>.Logger.Warning("UADMC smart refit: Ui.ConstructorUI target not found; button disabled.");
        return found;
    }
