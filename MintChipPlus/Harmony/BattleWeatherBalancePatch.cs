...
// MC's "Always Sunny" battle option keeps vanilla setup intact, then clamps
...
internal static class BattleWeatherBalancePatch
{
    ...
    internal static bool ApplyAlwaysSunny(string context, bool allowLoadingBattle = false, DayCycleAndWeather? weather = null, bool scheduleReadback = true)
    {
        ...
        try
        {
            ...
            if (beforeState != afterState || lastAppliedWeatherState != afterState)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC battle weather: forced Always Sunny via {context}. before={beforeState}; after={afterState}.");
            }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC battle weather balance failed; keeping vanilla weather. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
        finally
        {
    ...
    internal static bool ForceCalmWaves(string context, DayCycleAndWeather? weather = null)
    {
        ...
        try
        {
            ...
            if (!loggedCalmWaveCorrection)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC battle weather: enforcing calm waves via {context}. before={before:0.###}; after={weather.DesiredWavesPower:0.###}.");
            }
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle weather: calm-wave correction failed during {context}. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static IEnumerator RetryBattleWeatherWhenReady(string context, int generation)
    {
        ...
        if (ModSettings.BattleWeatherAlwaysSunny && (isBattleWeatherActive || GameManager.IsBattle))
            Melon<MintChipPlusMod>.Logger.Warning("UADMC battle weather: DayCycleAndWeather was not ready after battle-entry retries.");
    }
    ...
    private static IEnumerator ConfirmWeatherAfterReadbackDelay(string context, int generation)
    {
        for (int attempt = 1; attempt <= BattleWeatherReadbackAttempts; attempt++)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle weather: readback after {context} found non-sunny state; reapplying. state={state}.");
        ...
        if (generation == readbackGeneration && ModSettings.BattleWeatherAlwaysSunny && (isBattleWeatherActive || GameManager.IsBattle))
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC battle weather: could not read back final Always Sunny state after {context}.");
    }
    ...
    private static void LogWaitingForWeather(string context)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC battle weather: waiting for DayCycleAndWeather during {context}.");
    }
    ...
    private static void LogConfirmedAlwaysSunny(string context, string state)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC battle weather: confirmed Always Sunny via {context}. state={state}.");
    }
    ...
    private static void TryUpdateWeatherDetectionValues(string context)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC battle weather: skipped detection refresh during {context}; battle weather data was not ready. {ex.GetType().Name}: {ex.Message}");
        }
    }
