...
// availability layer. Enforce the same MC gates at Ship.IsPartAvailable.
...
internal static class DesignEarlyTbPartAvailabilityPatch
{
    ...
    private static void LogFirstBlock(string ruleKey, string ruleLabel, PartData part, int year, int unlockYear)
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC early TB availability: hiding {ruleLabel} before {unlockYear} in {ContextLabel()} constructor. year={year} firstBlocked={LogToken(partName)}.");
    }
    ...
    private static void LogUnknownYear()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            "UADMC early TB availability: could not resolve effective design year; leaving early TB part gates to vanilla/data unlocks.");
    }
