...
// state, rolls damage, calculates MC, and completes the battle.
[HarmonyPatch]
internal static class CampaignBattleAutoResolveOddsPatch
{
        ...
        try
        {
            ...
            if (summary != lastLoggedSummary)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC auto-resolve odds: displayed {playerChance:0}% player win chance for battle {battle.Id}.");
            }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC auto-resolve odds patch failed; leaving vanilla popup text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void BattlePopupUpdatePostfix(BattlePopup __instance)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC auto-resolve odds cleanup failed; leaving current popup text intact. {ex.GetType().Name}: {ex.Message}");
        }
    }
