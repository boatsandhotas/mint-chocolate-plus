...
internal static class CampaignDiplomacyActions
{
    ...
    internal static bool TriggerWar(Player? attacker, Player? defender)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC diplomacy: cannot trigger war. {reason}");
            return false;
        }
        ...
        // MC uses the vanilla attitude transition instead of writing relation
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC diplomacy: triggered war {attacker!.Name(false)} vs {defender!.Name(false)}; " +
    ...
    internal static bool ForcePeace(Player? player, Player? target)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC diplomacy: cannot force peace. {reason}");
            return false;
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC diplomacy: force peace begin {player!.Name(false)} vs {target!.Name(false)}; " +
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC diplomacy: force peace complete {player.Name(false)} vs {target.Name(false)}; " +
            $"attitude {beforeAttitude:0.0}->{relation.attitude:0.0}, war {beforeWar}->{relation.isWar}, " +
        ...
        out string reason)
    {
        ...
        if (victoryPointDelta <= ForcePeaceVictoryPointTolerance)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC diplomacy: force peace reparations {reason}.");
            return false;
        }
        ...
        if (winner == null || loser == null)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC diplomacy: force peace reparations {reason}.");
            return false;
        }
    ...
    private static bool InvokeTakeReparation(Relation relation, Player winner, Player loser, out string note)
    {
        if (TakeReparationMethod == null)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC diplomacy: force peace reparations {note}.");
            return false;
        }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC diplomacy: force peace reparations invoking vanilla TakeReparation; " +
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC diplomacy: force peace reparations finished; {note}, " +
