...
internal static class ShipEffectivePowerCalculator
{
    ...
    internal static EffectivePowerResult Calculate(Ship? design, EffectivePowerBenchmark? benchmark = null)
    {
        ...
        {
            LogFailureOnce("calculate:" + ex.GetType().Name, $"UADMC effective power: failed to calculate design power: {ex.Message}");
    ...
    private static void EnsureBenchmarksCurrent()
    {
        ...
        {
            LogFailureOnce("benchmark:" + ex.GetType().Name, $"UADMC effective power: failed to refresh benchmarks: {ex.Message}");
        }
    }
    ...
    // load-bearing for a lot of logic tuned against it. MC's own throw-weight model further
    ...
    private static float EstimateVanillaBasePower(Ship design)
    {
        ...
        {
            LogFailureOnce("base:" + ex.GetType().Name, $"UADMC effective power: vanilla EstimatePower failed: {ex.Message}");
            return 0f;
        }
    }

    // MC throw-weight power model — a DISPLAY-ONLY override for the Fleet UI's "Pwr" column.
    ...
    internal static float ComputeDisplayPower(Ship design, float fallbackAdjustedPower)
    {
        ...
        {
            LogFailureOnce("model:" + ex.GetType().Name, $"UADMC effective power: throw-weight model failed, using AI-facing power: {ex.Message}");
            return fallbackAdjustedPower;
        }
    }
    ...
    private static void AddTurretArmorScore(Ship ship, ref float weighted, ref float weights)
    {
        ...
        {
            LogFailureOnce("armor:turrets:" + ex.GetType().Name, $"UADMC effective power: turret armor score failed: {ex.Message}");
        }
    }
    ...
    internal static IReadOnlyList<GunGroupDiagnostic> DescribeGunGroups(Ship design)
    {
        ...
        {
            LogFailureOnce("describe:guns:" + ex.GetType().Name, $"UADMC power probe: gun-group describe failed: {ex.Message}");
        }
    ...
    private static IEnumerable<(PartData Data, int Count)> GunGroups(Ship ship)
    {
        ...
        {
            LogFailureOnce("guns:groups:" + ex.GetType().Name, $"UADMC effective power: gun group scan failed: {ex.Message}");
        }
        ...
        {
            LogFailureOnce("guns:parts:" + ex.GetType().Name, $"UADMC effective power: gun part scan failed: {ex.Message}");
        }
    ...
    private static IEnumerable<(PartData Data, int Count)> TorpedoGroups(Ship ship)
    {
        ...
        {
            LogFailureOnce("torps:list:" + ex.GetType().Name, $"UADMC effective power: torpedo list scan failed: {ex.Message}");
        }
        ...
        if (groups.Count == 0)
        {
            ...
            {
                LogFailureOnce("torps:parts:" + ex.GetType().Name, $"UADMC effective power: torpedo part scan failed: {ex.Message}");
            }
        }
    ...
    private static GunData? GetGunData(Ship ship, PartData data)
    {
        ...
        {
            LogFailureOnce("gundata:" + ex.GetType().Name, $"UADMC effective power: gun data lookup failed: {ex.Message}");
        }
    ...
    private static TorpedosData? GetTorpedoData(PartData data)
    {
        ...
        {
            LogFailureOnce("torpdata:" + ex.GetType().Name, $"UADMC effective power: torpedo data lookup failed: {ex.Message}");
        }
    ...
    private static float GunRange(Ship ship, PartData data)
    {
        ...
        if (fallback <= 1f)
            LogFailureOnce("range:gun:fallback", "UADMC effective power: gun range could not be resolved from GunData for at least one design; using neutral fallback.");
    ...
    private static float TorpedoRange(Ship ship, PartData data)
    {
        ...
        LogFailureOnce("range:torpedo:fallback", "UADMC effective power: torpedo range could not be resolved from weapon or tech-effect data for at least one design; using neutral fallback.");
        return 1f;
    }
    ...
    private static void LogTorpedoAuditMismatchIfAny(Ship design, TorpedoProfile torpedoes)
    {
        ...
        try
        {
            ...
            LogFailureOnce(
                ...
                $"UADMC effective power: torpedo tube mismatch for {label}; powerTubes={torpedoes.TubeCount} but torpedo audit is clean; source={TorpedoStateSignature(design)}.");
        }
        catch
        {
        }
    ...
    // Per-gun-group row used by the MC throw-weight display power model.
