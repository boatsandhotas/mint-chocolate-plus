...
internal static class ResupplyOverride
{
    ...
    internal static bool ResupplyShip(Player player, Ship ship)
    {
        ...
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC resupply: fuel failed — {ex.GetType().Name}: {ex.Message}"); }
        try { CampaignController.ReplenishAmmo(player, ship, HugeCapacity, 1f); ok = true; }
        catch (Exception ex) { Melon<MintChipPlusMod>.Logger.Warning($"UADMC resupply: ammo failed — {ex.GetType().Name}: {ex.Message}"); }
        return ok;
    }
    ...
    internal static int ResupplyAll()
    {
        ...
        try
        {
            ...
            if (player == null)
            {
                Melon<MintChipPlusMod>.Logger.Warning("UADMC resupply: no human player found.");
                return 0;
            }
            ...
            Melon<MintChipPlusMod>.Logger.Msg($"UADMC resupply: refueled + rearmed {n} ship(s).");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC resupply all failed: {ex.GetType().Name}: {ex.Message}");
        }
        return n;
    }
}
