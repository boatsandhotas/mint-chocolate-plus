...
// All logging under "UADMC_REINFORCE".
internal static class LandInvasionSupport
{
    private const string Feature = "land_reinforce";
    private const char RS = '', US = '';

    internal static void Log(string m) => Melon<MintChipPlusMod>.Logger.Msg("UADMC_REINFORCE " + m);
    ...
    internal static void DumpBattleDrivers()
    {
        try
        {
            ...
            Log("=== UADMC_REINFORCE BATTLE DRIVERS (your committed battles) ===");
            int nn = 0;
            foreach (var kv in battles)
            {
