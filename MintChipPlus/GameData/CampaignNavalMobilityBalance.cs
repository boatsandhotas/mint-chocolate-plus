...
// as a MC preset. CampaignShipsMovementManager.DistancePerTurn reads this param,
...
internal static class CampaignNavalMobilityBalance
{
    ...
    internal static void ApplyCurrentSetting(string context = "manual", UadGameData? gameDataOverride = null)
    {
        ...
        if (gameData?.paramsRaw == null && gameData?.parms == null)
        {
            if (!loggedMissingGameData)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg("UADMC campaign naval mobility: option stored; game params are not loaded yet.");
            }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC campaign naval mobility: applied {ModSettings.CampaignNavalMobilityModeText(mode)} during {context}; {summary}.");
    }
    ...
    private static bool TryWriteRawParam(UadGameData gameData, float value)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign naval mobility: failed to write raw param {MonthlyShipSpeedModParam}. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static bool TryWriteParsedParam(UadGameData gameData, float value)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC campaign naval mobility: failed to write parsed param {MonthlyShipSpeedModParam}. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static void LogMissingParam()
    {
        ...
        Melon<MintChipPlusMod>.Logger.Warning(
            $"UADMC campaign naval mobility: missing loaded param {MonthlyShipSpeedModParam}; cannot apply campaign mobility preset.");
    }
