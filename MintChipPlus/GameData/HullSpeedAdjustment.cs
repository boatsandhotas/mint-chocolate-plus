...
// Balance intent: reduce early small-craft hull speed targets that MC's
...
internal static class HullSpeedAdjustment
{
    ...
    internal static void ApplyCurrentSetting(string context = "manual", UadGameData? gameDataOverride = null)
    {
        ...
        if (gameData?.parts == null)
        {
            if (!loggedMissingGameData)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg("UADMC hull speed adjustment: option stored; part data is not loaded yet.");
            }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC hull speed adjustment: applied {ModSettings.HullSpeedAdjustmentModeText(enabled)} during {context}; TB {tbSeen}/3 {action} to {(enabled ? Fmt(AdjustedTbSpeedKnots) + " kn" : "vanilla")}; DD {ddSeen}/8 {action} to {(enabled ? Fmt(AdjustedDdSpeedKnots) + " kn" : "vanilla")}; TB funnel gate {funnelGate.Detail}; {string.Join(", ", changes)}.");
    }
    ...
    private static void LogMissingHull(string hullId)
    {
        if (LoggedMissingHulls.Add(hullId))
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC hull speed adjustment: missing loaded hull {hullId}; cannot adjust speed limiter.");
    }
    ...
    private static void LogMissingFunnel()
    {
        if (!loggedMissingFunnel)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC hull speed adjustment: missing loaded part {EarlyTbDualFunnelId}; cannot gate early TB dual funnel.");
        }
    }
