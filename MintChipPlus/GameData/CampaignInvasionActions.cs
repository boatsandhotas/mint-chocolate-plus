...
internal static class CampaignInvasionActions
{
        ...
        out string reason)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC naval-invasion: AreaCurrentTonnage check threw, allowing invasion. {ex.GetType().Name}: {ex.Message}");
        }
    ...
    internal static bool LaunchNavalInvasion(Player attacker, PortElement target)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC naval-invasion: refused. {reason}");
            return false;
        }
        ...
        if (!vanillaCreated)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC naval-invasion: vanilla path didn't create event; calling CreateConquestEvent manually " +
            ...
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC naval-invasion: CreateConquestEvent threw. {ex.GetType().Name}: {ex.Message}");
                return false;
            }
        }
    ...
    private static int ComputeRequiredTonnage(Player defender, Province province)
    {
        ...
        try
        {
            ...
            if (portSize > 0f)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC naval-invasion: province.Port heuristic ({province.Id}) = {portSize:0} → using {rounded} tons.");
                return rounded;
            }
        }
        catch { }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC naval-invasion: all vanilla tonnage methods unavailable; using default {DefaultRequiredTonnage} tons.");
    ...
    private static bool TryVanillaNavalInvasion(Player attacker, Player defender, Province province, int conquestCountBefore)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                "UADMC naval-invasion: vanilla CheckNavalInvasions(owner=defender, enemy=attacker) created the event.");
            return true;
        }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                "UADMC naval-invasion: vanilla CheckNavalInvasions(owner=attacker, enemy=defender) created the event.");
            return true;
        }
    ...
    private static bool TryCheckNavalInvasions(CampaignController cc, Player owner, Player enemy, Province province, string label)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC naval-invasion: CheckNavalInvasions({label}) threw. {ex.GetType().Name}: {ex.Message?.Split('\n')[0]}");
            return false;
        }
    }
    ...
    private static void TryRefreshTonnage(CampaignConquestEvent evt, PlayerData playerData, string label)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC naval-invasion: CheckEventRequiredTonnageFor({label}) threw. {ex.GetType().Name}: {ex.Message?.Split('\n')[0]}");
        }
    }
    ...
    private static int? TryTonnage(string label, Func<float> getter, Area area)
    {
        try
        {
            ...
            if (t > 0f)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC naval-invasion: {label}({area.Id}) = {t:0} → using {rounded} tons.");
                return rounded;
            }
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC naval-invasion: {label} threw, trying next. {ex.GetType().Name}: {ex.Message?.Split('\n')[0]}");
        }
        return null;
    }
    ...
    private static void RefreshConquestEventTonnage(Player attacker, PortElement port)
    {
        try
        {
            ...
            if (before != after)
                Melon<MintChipPlusMod>.Logger.Msg(
                    $"UADMC naval-invasion: CheckEventRequiredTonnageFor refined requiredTonnage {before} → {after}.");
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC naval-invasion: tonnage refresh failed. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static bool LaunchLandInvasion(Player attacker, Province target)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC land-invasion: refused. {reason}");
            return false;
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC land-invasion: creating ProvinceBattle attacker={attacker.Name(false)} " +
        ...
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC land-invasion: pre-AddBattle vanilla checks — HasEqualBattle={hasEqual} SameAttacker={sameAttacker}.");

            if (hasEqual)
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC land-invasion: vanilla HasEqualBattle says this attacker/defender pair " +
                    $"already has an equivalent battle; aborting to avoid duplication.");
                return false;
            }

            Melon<MintChipPlusMod>.Logger.Msg("UADMC land-invasion: calling ProvinceBattleManager.AddBattle…");
            try
            {
            ...
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC land-invasion: AddBattle threw. {ex.GetType().Name}: {ex.Message}");
                return false;
            }
            Melon<MintChipPlusMod>.Logger.Msg("UADMC land-invasion: AddBattle returned; calling AddArrow…");
            ...
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC land-invasion: AddArrow failed; removing the just-added battle to avoid " +
                    $"a LateUpdate crash. {ex.GetType().Name}: {ex.Message}");
                try { ProvinceBattleManager.RemoveBattle(battle); }
                catch { }
                return false;
            }
            Melon<MintChipPlusMod>.Logger.Msg("UADMC land-invasion: AddArrow returned; running diplomatic consequences…");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC land-invasion: ProvinceBattleManager.AddBattle threw. {ex.GetType().Name}: {ex.Message}");
            return false;
        }

        ApplyConsequencesIfPeacetime(attacker, defender, target, wasAlreadyAtWar, "land-invasion");
        Melon<MintChipPlusMod>.Logger.Msg("UADMC land-invasion: consequences applied; calling TryRefreshUi…");
        TryRefreshUi();
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC land-invasion: COMPLETED attacker={attacker.Name(false)} target={target.Id}.");
        return true;
    }
        ...
        Player attacker, Player defender, Player majorAlly, string source)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC {source}: skipping war trigger; already at war " +
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC {source}: minor {defender.Name(false)} is backed by {majorAlly.Name(false)}; " +
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC {source}: {defender.Name(false)} is an unprotected minor with no relation entry; " +
    ...
    private static bool ForceWarWithDefender(Player attacker, Player defender, Relation? relation, string source)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC {source}: cannot force war; no relation between {attacker.Name(false)} and {defender.Name(false)}.");
            return false;
        }
        ...
        try
        {
            CampaignController.Instance.AdjustAttitude(
                ...
                true,        // canFullyAdjust
                false,       // init
                $"UADMC {source} declares war on defender",
                true,        // raiseEvents
                true,        // force
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC {source}: AdjustAttitude failed forcing war on {defender.Name(false)}. " +
                $"{ex.GetType().Name}: {ex.Message}");
            return false;
        }

        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC {source}: forced war {attacker.Name(false)} → {defender.Name(false)}; " +
        ...
        Player attacker, Player defender, Province province, bool wasAlreadyAtWar, string source)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC {source}: diplomatic consequences threw. {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    internal static bool BreakAllianceIfPresent(Player attacker, Player defender, string source)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC {source}: clearing isAlliance threw. {ex.GetType().Name}: {ex.Message}");
        }
        ...
        try
        {
            CampaignController.Instance?.AdjustAttitude(
                ...
                true,        // canFullyAdjust
                false,       // init
                $"UADMC {source}: invaded sworn ally",
                true,        // raiseEvents
                true,        // force
                false);      // fromCommonEnemy
        }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC {source}: alliance-break attitude hit threw. {ex.GetType().Name}: {ex.Message}");
        }

        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC {source}: BROKE ALLIANCE with {defender.Name(false)}; " +
    ...
    private static Province? FindBorderingAttackerProvince(Player attacker, Province target)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC land-invasion: {target.Id} has no land neighbours (likely an isolated island); blocking.");
            return null;
        }
        ...
        foreach (Province p in mine)
        {
            ...
            foreach (Province mn in myNeighbours)
                ...
                {
                    Melon<MintChipPlusMod>.Logger.Msg(
                        $"UADMC land-invasion: mutual land border confirmed {p.Id} ↔ {target.Id}.");
                    return p;
                }
        }

        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC land-invasion: no mutual land border between attacker and {target.Id}; blocking.");
        return null;
    }
        ...
        Player attacker, Player defender, Province? attackerProvince, Province target)
    {
        ...
        catch (Exception ex)
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC land-invasion: HasEqualBattle threw for " +
        ...
        {
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC land-invasion: HasEqualBattle=TRUE for " +
                $"{attacker.Name(false)}→{defender.Name(false)}@{target.Id}; " +
