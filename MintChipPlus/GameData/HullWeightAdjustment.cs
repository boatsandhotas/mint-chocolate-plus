    ...
    internal static void ApplyCurrentSetting(string context = "manual", UadGameData? gameDataOverride = null)
    {
        ...
        if (gameData?.parts == null)
        {
            if (!loggedMissingGameData)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg("UADMC hull weight adjustment: option stored; part data is not loaded yet.");
            }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC hull weight adjustment: applied {ModSettings.HullWeightAdjustmentModeText(enabled)} during {context}; {summary}.");
    }
