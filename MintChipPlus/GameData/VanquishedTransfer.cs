...
internal static class VanquishedTransfer
{
    ...
    internal static void OnDisablePlayer(CampaignController cc, Player dying)
    {
        ...
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC vanquished spoils: {dyingName} eliminated — victors={FormatVictors(weight)} finisher={(string.IsNullOrEmpty(lastFinisher) ? "none" : lastFinisher)} ships={n} (transferred={transferred} scuttled={scuttled}) cashSeized={cashMoved:0}.");
        }
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC vanquished spoils failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static bool TryScuttle(CampaignController cc, Ship ship)
    {
        ...
        {
            try { ship.Sink("UADMC vanquished"); return true; }
            catch { return false; }
        }
    }
