...
// campaign Guid is reachable at runtime). This honors MC's no-loose-config rule,
...
internal static class ModCampaignState
{
    ...
    internal static void EnsureCampaignId()
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC campaign-id: stamped main-player aiName GUID {(guid.Length >= 8 ? guid.Substring(0, 8) : guid)} (none was present; persists on next save).");
        }
        catch
        {
        }
    ...
    private static string PrefKey(string feature, string campaignKey)
        => $"uadmc_state_{feature}_{campaignKey}";
