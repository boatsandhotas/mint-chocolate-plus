...
internal static class ShipWeightBalance
{
    ...
    internal static void ApplyCurrentSetting(string context = "manual", UadGameData? gameDataOverride = null)
    {
        ...
        if ((gameData?.paramsRaw == null && gameData?.parms == null) || gameData?.parts == null)
        {
            if (!loggedMissingGameData)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg("UADMC ship weight balance: option stored; game params or part data are not loaded yet.");
            }
        ...
        Melon<MintChipPlusMod>.Logger.Msg(
            $"UADMC ship weight balance: applied {ModSettings.HullWeightAdjustmentModeText(enabled)} during {context}; {summary}.");
    }
    ...
    private static bool TryWriteRawParam(UadGameData gameData, string paramName, float value)
    {
        ...
        catch (Exception ex)
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship weight balance: failed to write raw param {paramName}. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static bool TryWriteParsedParam(UadGameData gameData, string paramName, float value)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning(
                $"UADMC ship weight balance: failed to write parsed param {paramName}. {ex.GetType().Name}: {ex.Message}");
            return false;
        }
    }
    ...
    private static void LogMissingParam(string paramName)
    {
        if (LoggedMissingParams.Add(paramName))
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC ship weight balance: missing loaded param {paramName}; cannot adjust it.");
    }

    private static void LogMissingTower(string id)
    {
        if (LoggedMissingTowers.Add(id))
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC ship weight balance: missing TB tower part {id}; cannot adjust it.");
    }
