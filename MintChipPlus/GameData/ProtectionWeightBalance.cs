    ...
    internal static void ApplyCurrentSetting(string context = "manual", UadGameData? gameDataOverride = null)
    {
        ...
        if (gameData?.technologies == null)
        {
            if (!loggedMissingGameData)
            {
                ...
                Melon<MintChipPlusMod>.Logger.Msg("UADMC protection weight balance: technology data is not loaded yet.");
            }
        ...
        foreach (var entry in gameData.technologies)
        {
            ...
            catch (Exception ex)
            {
                Melon<MintChipPlusMod>.Logger.Warning(
                    $"UADMC protection weight balance: failed to postprocess {techKey} after rewrite. {ex.GetType().Name}: {ex.Message}");
            }
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC protection weight balance: {summary}.");
    }
