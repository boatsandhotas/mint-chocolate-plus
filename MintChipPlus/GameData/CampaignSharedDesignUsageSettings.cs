...
// This is campaign save-state, not a MC PlayerPrefs option. The active
...
internal static class CampaignSharedDesignUsageSettings
{
    ...
    internal static bool TrySetMode(SharedDesignPolicy policy)
    {
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning("UADMC option: Shared Designs cannot be changed without a loaded campaign.");
            return false;
        }
        ...
        Melon<MintChipPlusMod>.Logger.Msg($"UADMC option: Shared Designs mode {ModeText(oldPolicy)} -> {ModeText(policy)}.");
