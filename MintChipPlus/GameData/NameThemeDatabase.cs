...
internal static class NameThemeDatabase
{
    ...
    internal static void EnsureLoaded()
    {
        ...
        try
        {
            ...
            {
                Melon<MintChipPlusMod>.Logger.Warning("UADMC naming-db: embedded ship_names.csv not found; themes unavailable.");
                return;
            }
        ...
        {
            Melon<MintChipPlusMod>.Logger.Warning($"UADMC naming-db: load failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private static void LogLoadSummary(int rows)
    {
        try
        {
            ...
            Melon<MintChipPlusMod>.Logger.Msg(
                $"UADMC naming-db: loaded {rows} names — countries({countries.Count})=[{string.Join(",", countries)}] universalThemes={universalThemes} cultureGated={_themeCultures!.Count}.");
        }
        catch
        {
        }
